...
technique as a library, \selfassembly, for Scala, and evaluate its
practicality by migrating a full-featured industrial-strength serialization
framework for Scala to \selfassembly.
\end{abstract}
...
printed string representation of the value.
Since the need to add functionality ubiquitously across types is so pervasive,
it's advantageous to have some programming language support
...
it's common to have to override them to achieve desired functionality.
...
that functionality can be implemented \emph{after} the type or
...
definition of the class. A major benefit of retroactive extensibility is
flexibility and possibility for customizing behavior. As a result,
several authors have argued for the software
engineering benefits of using type classes~\cite{}.
Because of the flexibility and extensibility of type classes, the
Scala has embraced them~\cite{ScalaCollections,Pickling}.
...
programmer to write any serialization boilerplate code at all.
...
One could also imagine that type classes could be useful for
providing operations such as \lstinline{equals} or
\lstinline{toString}. In all of these cases, it would be advantageous
...
While built-in \emph{language extensions} like Java
serialization are solve this inconvenience by requiring
...
functional language, which has considerable differences to OO languages
