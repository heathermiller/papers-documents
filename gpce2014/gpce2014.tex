...
high-level overview of the pattern. macros generate calls to implicity and generate the bodies of implicit objects. the macro also inspects the type and based on that, generates implicitly calls, which in turn are implicitly expanded.
...
\end{lstlisting}
\noindent
Thus, the implicit lookup mechanism inserts an invocation of the macro method \verb|genQuery|. Being a macro, \verb|genQuery| operates on abstract syntax trees instead of runtime values. It is declared as follows:
...
\end{lstlisting}
\noindent
Note that in this declaration the type parameter \verb|T| is annotated with a so-called \emph{context bound} \verb|c.WeakTypeTag|. The context bound is an alternative way of adding a synthetic implicit parameter:
