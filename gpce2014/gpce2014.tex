...
\item {\bf \sselfassembly:} A framework for generic programming based on 
  type classes and type-safe macros.

\item the pattern isn't only good for standard DGP (that also happens
...
\item in all of this, we additionally deal with oo issues like
  subtyping polymorphism (we generate type classes for all known
  subtypes and generate subtyping dispatches) and object
  identity/cycles.
