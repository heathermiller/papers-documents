...
\begin{abstract}
  ...
  language built-in mechanisms (such as the Java serialization
  mechanism), LLEs are \emph{user-definable}, \emph{multi-purpose} (they can be used to define
  ...
  mechanism for doing automatic implementations of \emph{type classes}
  using a \emph{type-safe} macro mechanism. We implemented our
  technique as a library, \sselfassembly, for Scala, and evaluated its
...
Scala. These datatypes can be described using the grammar in
...
extended; extension is deprecated, though.} In subsequent sections 
...
that type. We structure our treatment into three distinct steps: 
\emph{triggering generation}
(Section~\ref{sec:triggering-generation}); 
\emph{macro-based generation technique}
(Section~\ref{sec:macro-based-generation});
\emph{examples and relation to code in Section~\ref{sec:type-classes}} (Section~\ref{sec:generated-type-class-instances}).
% (described in the
...
The body of the type class is generated using:\footnote{For simplicity and clarity of presentation 
we are not deal with hygiene.}
...
\noindent The quasiquote produces an AST which implements the
single abstract method of the type class. The AST returns the value of
\verb|combineResult|, a local variable of type \verb|R| and the type parameter of
\verb|Query[R]| within which the macro is defined. \verb|R| is the return type
of the type class method (\verb|String| in the case of \verb|Show|). The
value of \verb|combineResult| is the result of combining:
1) the initial result returned by the AST \verb|trees.first(tpe)|; 2)
the results returned by nested invocations of the type class on fields;
3) the result returned by the AST \verb|trees.last(tpe)|. To obtain
...
Let's say the datatype that we'd like to call \verb|show| is the
\lstinline{Tree} type on Figure~\ref{fig:trees}.
%\begin{lstlisting}
%sealed trait Tree
%case class Fork(left: Tree, right: Tree)
% extends Tree
...
%\end{lstlisting}
%\noindent
In order to create type class instances of type
...
generated for an
arbitrary shape $C$. The implicit object (1) is
