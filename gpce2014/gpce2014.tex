...
First, we consider a datatype whose declaration has the shape:
...
The next step is creating instances for (abstract) traits of the following
shape:
\begin{math}
...
Queries are defined by extending either of the traits \verb|Query| or
\verb|CyclicQuery|. The latter trait is designed to handle cyclic object
graphs. Figure~\ref{fig:define-query} shows how to define \verb|Show[T]| as a
cyclic query. Extending \verb|CyclicQuery|, the \verb|Show| companion object
is required to implement the factory method \verb|mkTrees|, which returns an
instance of \verb|Trees[C]|; \verb|Trees[C]| provides a number of methods that
are invoked by the macro at compile time to obtain AST fragments that are
...
\verb|q"$left + $right"| denotes a quasiquote, creating an AST node using
Scala code, where \verb|left| and \verb|right| are tree nodes that are
spliced into the result node. (The variation using triple quotation marks
treats the string verbatim, so that quotation marks do not have to be
escaped.)
...
method that should be used by the macro.}
...
\end{lstlisting}
  \caption{Defining a query using \texttt{selfassembly}}
  \label{fig:define-query}
\end{figure}
