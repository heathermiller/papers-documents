...
analysis like a \emph{immutability checker}. However unlike language
built-in mechanisms (such as the Java serialization mechanism), LLEs
are multi-purpose (they can be used to define various forms of
generic functionality) and highly customizable and extensible.  The
key idea is to provide programmers with a generic mechanism for
doing automatic implementations of \emph{type classes} using a
type-safe macro mechanism. We've implemented our technique as a
library, \selfassembly, for Scala, and evaluate its practicality by
migrating a full-featured industrial-strength serialization
...
\lstinline{PTree[Tree]}. To print the trees the instance for
