...
coordination with observables. Observables, as assumed here, follow the so-
called  ``Reactive Extension Contract''~\cite{RxContract:2010}. This means
...
help of an example. Consider the following join pattern involving observables
...
The \texttt{Next} messages are buffered in lock-free queues which provide non-
blocking, thread-safe enqueuing, dequeuing, and peeking at the queue head.
...
a pattern means successfully claiming, that is changing the status of a
message from \texttt{Pending} to \texttt{Claimed}, a message for every event
