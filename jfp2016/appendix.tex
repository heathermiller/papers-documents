...
\begin{lem}[Responsive FMap]\label{lem:resp-fmap}
\end{lem}
\begin{proof}
\begin{enumerate}
  % 1.
\item By \textsc{R-Local} and \textsc{R-FMap}
  \begin{enumerate}[label=(\alph*)]
  \item $E[\texttt{flatMap}(r, p)]~|~\sigma \rightarrow^h E[r']~|~\sigma~|~\{ r' \}$
  \item $r' = {\Ref {l'} {h'}}$
  \item $r = {\Ref l {h'}}$
...
\item By 9.a,b), and \textsc{T-Spore}
  ...
  \item By the type rules, derivation 10.a) does not contain
    ...
    \texttt{flatMap} invocation. Therefore, by
    ...
    {\em Case 2:} $d > 0$. The depth of nested \texttt{flatMap}s of
    term $p~v$ is less than the depth of the term $\texttt{flatMap}(r,
    p)$.  Therefore, by the induction hypothesis, reductions of nested
    \texttt{flatMap} invocations within $p~v$ result in responsive
    ...
    result of a nested \texttt{flatMap} invocation,
