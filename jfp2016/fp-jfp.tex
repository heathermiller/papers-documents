...
\begin{abstract}
  ...
  {\em \FP} designed to provide a more principled substrate (think middleware) upon
...
meant to underly systems like Spark. Function passing adopts the concept of a 
...
our model from a high-level, elaborating
...
its resulting computation to the current node.
...
express \verb|map| in terms of \verb|apply|:
...
\noindent
This should come as no surprise, given that \verb|apply| is the monadic bind
...
Our ongoing efforts are three-fold; we're working on a semantics and implementation 
of fault handling on top of the \FP~model, we are exploring approaches for memory 
reclamation, and we're working to better understand the concerns of separate compilation. 
...
primitives are overloaded. For example, consider the running example illustrated 
inFigure~\ref{fig:bigger-dag}, but extended with a fault handler:
