...
\begin{itemize}
  ...
  \item A distributed implementation of the programming model in and
    for Scala as a middleware.\footnote{See the Git repository at
      ...
      \texttt{jfp}.} In addition, we present prototype versions we
    have built of popular frameworks like Apache Spark and
    MBrace~\cite{MBrace} using the function passing model, and
    end-user applications we have built using each of these prototype
    frameworks.
...
proof of a subject reduction theorem in Section~\ref{sec:subject-reduction}. Finally,
...
the \verb|flatMap| method of \verb|SiloRef|.
