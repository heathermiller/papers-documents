...
\comment{\todo{In order to subsume the subsequent subsections, I'd rename this section
...
\comment{Using each example system, we've implemented several example
applications, excerpts of some of which are explained below.}
...
Systems like Apache Spark~\cite{Spark}, MapReduce~\cite{MapReduce},
and Dryad~\cite{Dryad} are distributed systems for large-scale data
processing, building on concepts from functional programming, such as
higher-order functions. DryadLINQ~\cite{YuIFBEGC08} extends Dryad with
a functional language, LINQ, for expressing transformations on
distributed datasets which enables sophisticated optimizations such as
those typically employed by databases. The data model of DryadLINQ is
strongly typed, another aspect shared by the function passing
model. Nectar~\cite{Nectar} provides a caching service to improve the
resource utilization of Dryad/DryadLINQ clusters. For this, Nectar
identifies derived datasets by the computations that generate them,
similar to lineages in the function passing model. Nectar could
significantly improve space utilization as well as provide speed-ups
via incremental computation and shared sub-computations. It would be
interesting to investigate potential usages of the lineages of the
functional passing model for similar purposes. In contrast to these
systems, the function passing model is meant to act as more of a
middleware to facilitate the design and implementation of such
systems, and as a result provides finer-grained control over details
such as fault handling. Rather than system building and experimental
evaluation, our focus is on a precise formalization of the programming
model, as well as the proof of preservation and progress properties.
