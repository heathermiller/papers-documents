...
{ host(r) = h' \quad i~\mbox{fresh} \quad r' = \mbox{Mapped}((h, i), h', r, p, \mbox{None}) }
{ (R[\texttt{map}(r, p)], E, S)^h \longrightarrow (R[r'], E, S)^h }

%\inferrule[\textsc{R-FMap}]
%{ host(r) = h' \quad i~\mbox{fresh} \\
%  r' = \mbox{FMapped}((h, i), h', r, p, \mbox{None})
%}
...
%{ E = \mbox{Res}(\iota, v) \texttt{::} E' \quad S' = S + (\iota \mapsto v)
%}
%{ (R[\texttt{await}(\iota_f)], E, S)^h \longrightarrow (R[\texttt{await}(\iota_f)], E', S')^h }
%
%\inferrule[\textsc{R-ReqLocal}]
%{ E = \mbox{Req}(h', r, \iota'') \texttt{::} E' \quad r = \mbox{Mapped}(\iota,
%  h, r', p, \mbox{None}) \quad r' \neq \mbox{Mat}(\iota_s) \quad S(\iota) =
%  \mbox{None} \\
%  loc(r') = \iota' \quad S(\iota') = \mbox{None} \\
%  E'' = \mbox{Req}(h, r', \iota') \texttt{::} E
%}
...
\mbox{Some}(v)$ to express the fact that $S$ maps $\iota$ to $v$. We write
$S(\iota) = \mbox{None}$ if $S$ does not have a mapping for $\iota$. Reduction
