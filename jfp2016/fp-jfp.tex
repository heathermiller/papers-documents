...
\author[P. Haller, H. Miller, and N. M{\"u}ller]{%
  ...
  HEATHER MILLER\\ EPFL, Switzerland\\[1ex]%\email{heather.miller@epfl.ch}
...
data in the silo associated with the given \verb|SiloRef|. Rather than
immediately sending the spore across the network, and waiting for the
operation to finish, the \verb|apply| method's evaluation is {\em
  deferred}. Without involving any network communication, it
immediately returns a \verb|SiloRef| referring to a new, to-be-created
silo. This new silo reference only contains lineage information,
namely, a reference to the original \verb|SiloRef|, a reference to the
argument spore, and the information that it is the result of an
\verb|apply| invocation. As we explain below, another method,
\verb|send|, must be called explicitly to force the materialization of
the result silo. Note that the result type of the spore parameter is a
\verb|SiloRef[S]|. Semantically, the new silo created by \verb|apply|
is defined to contain the data of the silo that the user-defined spore
returns. This way, the \verb|apply| combinator enables expressing
computation DAGs.
...
following application of \verb|apply| defines a (not-yet-materialized)
