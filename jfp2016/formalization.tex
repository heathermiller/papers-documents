...
\gap ~|~  \text{Applied}(\iota, l, p)                  & \mbox{lineage with \texttt{apply}} \\
                                                       & \\
...
\ea$
\caption{Abstract syntax of core language. Integer operators are
  represented using $\oplus \in \{ \texttt{+}, \texttt{-}, \texttt{*},
  \texttt{/} \}$.}\label{fig:syntax}
\end{figure}
...
initializes a new silo on a given host with a given value. The
\verb|apply| term creates a lineage of silo transformations
represented as a silo reference (see below).  The \verb|send| term
forces the materialization of the argument silo. The \verb|send|
expression returns a future which is asynchronously completed with the
silo's value. The \verb|await| term waits for the completion of its
argument future and returns the future's value. Decentralized
identifiers $\iota$ are used to refer to futures and to identify silos
via their lineages (each element in a lineage carries a decentralized
identifier).

Values in our language are as expected: besides abstractions and integer literals they
...
Rule \textsc{R-Apply} creates a lineage using the
constructor $\mathit{Applied}$. The new lineage has a fresh identifier $(h,
i)$ which uniquely identifies the corresponding (logical) silo. The
spore value $p$ is stored in the new lineage; this enables a
materialization of the silo identified by $(h, i)$ using parent
lineage $l$ and spore $p$.
...
\ea$
\caption{Messages.}\label{fig:messages}
\end{figure}
...
M'$ are multisets of messages. A host with identifier $h$ in $H$ has
the form $(t, \sigma)^h$. $t$ is the term reduced by $h$; $\sigma$ is
the local store of $h$. The multisets $M, M'$ model {\it message
  sends} that are currently ``in flight;'' a message send $h
\leftarrow m$ expresses the sending of message $m$ to host $h$. If $h
\leftarrow m \in M$ then message $m$ has not yet been delivered to $h$
...
Rule \textsc{R-Populate} asynchronously populates a new silo on host
...
enabled. Rule \textsc{R-Process-Val} is analogous to rule
\textsc{R-Process} for the case where a host $h$ is unable to reduce
its term further.
...
materialized (i.e., host $h$ is still waiting for a message ${\Res
...
\inferrule[\textsc{T-Int}] {
  ...
  \Gamma ; \Sigma \vdash n : \texttt{Int}
}

\inferrule[\textsc{T-IntOp}] {
  \Gamma ; \Sigma \vdash t : \texttt{Int} \quad
  \Gamma ; \Sigma \vdash t' : \texttt{Int}
} {
  \Gamma ; \Sigma \vdash t~\oplus~t' : \texttt{Int}
}
...
silo. Rule \textsc{WF-Lin2} requires the types of $\iota$ and $id(l)$
...
(e.g., $\mathit{Applied}$), $\iota$ is the identifier of lineage $l'$.
