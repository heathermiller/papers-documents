...
\verb|flatMap| term creates a lineage of silo transformations
...
host. Lineages are values of a simple datatype with constructors {\em
  Mat} and {\em FMapped}. The constructors include all information
...
Rule \textsc{R-FMap} creates a lineage using the
constructor {\em FMapped}. The new lineage has a fresh identifier $(h,
...
Rule \textsc{Proc-ReqFMap} is only enabled when the requested silo $r$
...
$\text{FMapped}$ constructor. The materialization of $r$ consists of
...
\textsc{Proc-ReqFMap} returns a term containing the application $p~v$
...
\textsc{T-Populate} and \textsc{T-FMap} are
straightforward; note that \verb|flatMap| is
...
(e.g., $\text{FMapped}$), $\iota$ is the identifier of lineage $l'$.
...
Rule \textsc{Proc-ReqFMap} is extended to
