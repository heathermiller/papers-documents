...
& \\
l ::=                                                  & \mbox{{\it{lineages:}}}              \\
\gap \:\:\:\: \text{Mat}(\iota, v)                     & \mbox{materialized}                  \\
...
\inferrule[\textsc{R-Populate}] {
  l = {\Mat {(h, i)} v} \quad i~\text{fresh}
} {
  E[\texttt{populate}(h', v)]~|~\sigma
  \rightarrow^h
  E[{\Ref l {h'}}]~|~\sigma
}
...
lineage ${\Mat {\iota'} v}$; thus, a response ${\Res \iota v}$ is
...
\inferrule[\textsc{WF-Lin3}] {
  \iota \in dom(\Sigma) \quad
  \emptyset ; \Sigma \vdash v : T
} {
  \Sigma \vdash {\Mat \iota v}
}
...
\begin{defn}[Responsive Configuration]\label{def:resp-config}
  ...
  $\forall (t, \sigma, Q \cdot m)^h \in H.~H = \{ (t, \sigma, Q \cdot
  m)^h \} \cup H' \Longrightarrow H~|~M \twoheadrightarrow^* \{ (t', \sigma',
  m \cdot Q')^h \} \cup H'~|~M'$ after a finite number of reduction
  steps.
\end{defn}
...
\begin{proof}[Proof Sketch]
  By Lemma~\ref{lem:strong-normal} and the definition of function
  $\mathit{process}$ (see
...
\begin{proof}[Proof Sketch]
  ...
  \begin{itemize}
    \item Case $r = {\Ref {\Mat {\iota'} v} h}$. The result follows from
