...
callbacks, and (b) to recover silos on faulty hosts by materializing
...
configurations. An extended host configuration has the following form:
$(t, \sigma, Q, F)^h$ where components $t$, $\sigma$, $Q$, and $h$ are
...
\inferrule[\textsc{R-Send}] {
  ...
  m = {\Req h r {id(l)} } \quad
  M' = M \cup \{ h'' \leftarrow m \} \\
  F' = F \cup \{ h'' \rightarrow ({\Req h {\Ref l {h'}} {id(l)} }, h') \}
} {
  \{ (E[\texttt{send}(r, h')], \sigma, Q, F)^h \} \cup H ~|~ M
  \twoheadrightarrow
  \{ (E[id(l)], \sigma, Q, F')^h \} \cup H ~|~ M'
}
\end{mathpar}
...
\cite{Hayashibara04} (among others, implemented in Akka~\cite{Akka}).
