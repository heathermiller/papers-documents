...
\inferrule[\textsc{R-Respond}] {
  M' = M \cup \{ h' \leftarrow {\Res \iota v} \}
} {
...
\inferrule[\textsc{Proc-ReqMat}] {
  ...
  process(h, {\Req {h'} r \iota}, \sigma) = (\texttt{unit}, \{ h' \leftarrow {\Res \iota {\texttt{unit}}} \}, \sigma)
}
...
\inferrule[\textsc{R-Respond}] {
  M' = M \cup \{ h' \leftarrow \highlight{{\ResP \iota v P}} \}
} {
  \{ (E[\highlight{\texttt{respond}(h', \iota, v, P)}], \sigma, Q)^h \} \cup H ~|~ M
  \twoheadrightarrow
  \{ (E[\texttt{unit}], \sigma, Q)^h \} \cup H ~|~ M'
}

\end{mathpar}
\caption{Extension of reduction.}\label{fig:reduction-persist}
\end{figure}
...
\inferrule[\textsc{Proc-Req}] {
  r = {\Ref l h} \quad
  \sigma(id(l)) = (v, P) \quad
  M = \{ h' \leftarrow {\ResP \iota v P} \} \quad
...
\inferrule[\textsc{Proc-ReqMat}] {
  ...
  process(h, {\Req {h'} r \iota}, \sigma) = (\texttt{unit}, \{ h' \leftarrow \highlight{{\ResP \iota {\texttt{unit}} \emptyset}} \}, \sigma)
}
...
\inferrule[\textsc{Proc-ReqMap}] {
  ...
  t = \highlight{\texttt{respond}(h', \iota, p~v, P)} \quad
...
\inferrule[\textsc{Proc-ReqFMap}] {
  ...
  l = {\FMapped {\iota'} {l'} p} \quad
  \sigma(id(l')) = \highlight{(v, P)} \\
  t = \highlight{\texttt{respond}(h', \iota, \texttt{await}(\texttt{send}(p~v)), P)} \quad
  \highlight{\sigma' = {\consume {id(l')} P \sigma}}
} {
  process(h, {\Req {h'} r \iota}, \sigma) = (t, \emptyset, \highlight{\sigma'})
}
...
\inferrule[\textsc{T-Respond}] {
  h \in \mathcal{H} \quad
  \Gamma ; \Sigma \vdash t : \texttt{Future}[T] \quad
  \Gamma ; \Sigma \vdash t' : T \quad
  \highlight{P \subseteq \mathcal{H}}
} {
  \Gamma ; \Sigma \vdash \texttt{respond}(h, t, t', \highlight{P}) : \texttt{Unit}
}
