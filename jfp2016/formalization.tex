...
Rule \textsc{R-Send} reduces a term $\texttt{send}(r, r')$ on host
...
is reduced to the identifier of $r$'s lineage, $id(l)$. The silo
reference $r'$ is used to identify the silo that should be
materialized using the requested value. In user programs,
\verb|send(r)| takes just one argument; this is equivalent to the term
$\texttt{send}(r, r)$ in the core language. The reason for the more
flexible variant of \verb|send| in the formal model is that it is used
for expressing the operational semantics of \verb|flatMap| (see
below).

Rule \textsc{R-Receive} moves an in-flight message send $h \leftarrow
m$ from $M$ to the local message queue of host $h$. Note that there
may be multiple in-flight message sends with target $h$ in $M$; the
message send is chosen non-deterministically in this case.
