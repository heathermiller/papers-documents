...
\inferrule[\textsc{Proc-ReqMat}] {
  r = {\Ref l h} \quad
  l = {\Mat {\iota'}} \quad
  t = \texttt{respond}(h', \iota, \texttt{unit})
} {
...
\comment{
Crucially, when
reading the value of a silo with identifier $\iota$ and persist set
$P$, i.e., $\sigma(\iota) = (v, P)$, the mapping for $\iota$ is
removed from resulting store $\sigma'$ if persist set $P$ is empty. An
empty persist set indicates that the corresponding silo has not been
persisted.  The removal of the mapping $\iota \mapsto (v, P)$ from
store $\sigma'$ is expressed using the $\mathit{consume}$ function
which checks for empty persist sets: $\sigma' = {\consume \iota P
  \sigma}$. The $\mathit{consume}$ function is defined as follows:}
