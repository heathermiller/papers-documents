...
Rule \textsc{R-Assign} reduces a term $\texttt{assign}(\iota, v)$ by
updating the store to map identifier $\iota$ to value $v$; this
corresponds to populating silo $\iota$ in the local store of host $h$.
...
Rule \textsc{R-Send} describes the reduction of a term
$\texttt{send}(r, r')$ on host $h$. The reduction initiates the
materialization of silo $r$ by sending a message $m = {\Req h r
  {id(l')}}$ to the host of $r$, $h'$. Thus, the message send $h'
\leftarrow m$ is added to the resulting set of in-flight messages
$M'$. The \verb|send| term itself is reduced to the identifier of
$r$'s lineage, $id(l)$. The silo reference $r'$ is used to identify
the silo that should be materialized using the requested value. In
user programs, \verb|send(r)| takes just one argument; this is
equivalent to the term $\texttt{send}(r, r)$ in the core language. The
reason for the more flexible variant of \verb|send| in the formal
model is that it is used for expressing the operational semantics of
\verb|flatMap| (see below).
...
\begin{mathpar}
\inferrule[\textsc{R-Await}] {
  \sigma(\iota) = \highlight{(v, P)} \quad \highlight{\sigma' = {\consume \iota P \sigma}}
} {
  E[\texttt{await}(\iota)]~|~\sigma
  \rightarrow^h
  E[v]~|~\highlight{\sigma'}
...
\end{mathpar}
\caption{Extension of local reduction.}\label{fig:local-reduction-persist}
\end{figure}
...
\inferrule[\textsc{Proc-Req}] {
  r = {\Ref l h} \quad
  \sigma(id(l)) = \highlight{(v, P)} \quad
  M = \{ h' \leftarrow \highlight{{\ResP \iota v P}} \} \quad
  \highlight{\sigma' = {\consume {id(l)} P \sigma}}
} {
  process(h, {\Req {h'} r \iota}, \sigma) = (\texttt{unit}, M, \highlight{\sigma'})
}
...
\inferrule[\textsc{Proc-ReqMap}] {
  ...
  t = \texttt{assign}(\iota', p~v) \quad
  \highlight{\sigma' = {\consume {id(l')} P \sigma}}
} {
  process(h, {\Req {h'} r \iota}, \sigma) = (t, \{ h \leftarrow {\Req {h'} r \iota} \}, \highlight{\sigma'})
}
...
\inferrule[\textsc{Proc-ReqPersist}] {
  ...
  \sigma(id(l')) = (v, P)                      \\
  P'             = P \star \{h''\}             \quad
  \sigma'        = [\iota' \mapsto (v, P')]\sigma  \quad
  \sigma''       = {\consume {id(l')} P {\sigma'}}
} {
  process(h, {\Req {h'} r \iota}, \sigma) = (\texttt{unit}, \{ h' \leftarrow {\ResP \iota v {P'}} \}, \sigma'')
}

\inferrule[\textsc{Proc-Res}] {
  \sigma' = [\iota \mapsto \highlight{(v, P)}]\sigma
} {
  process(h, \highlight{{\ResP \iota v P}}, \sigma) = (\texttt{unit}, \emptyset, \sigma')
}
\end{mathpar}
\caption{Extension of message processing.}\label{fig:process-persist}
\end{figure}
...
\inferrule[\textsc{WF-Q2}] {
  ...
  \Sigma \vdash \highlight{{\ResP \iota v P}} \cdot Q
}
\end{mathpar}
\caption{Extension of type assignment and well-formedness.}\label{fig:type-rules-persist}
\end{figure}
