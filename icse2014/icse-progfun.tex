...
by the students is executed in a restricted environment within each test case.
We may also add that we are not aware of any attempts to compromise our grading
infrastructure.


Scaling
  - given the number of students and our desire to provide them with feedback
    on their submission within 15 to 30 minutes, the use of a scalable cloud
    service was a natural choice.
  - our grading service is built on top of amazon web services (aws). they provide
    infrastructure to start up and terminate virtual machines automatically based
    on custom workload measures.
  - the virtual machines are configured to automatically start downloading and
    grading assignments and uploading the results once they are booted.


platform specific
  - code cannot easily be re-used
  - coursera  - they use SQS
  - test framework customizations - test framework for scala and java
  - sbt - build tool for scala and java
  - but concepts would work in other programming courses
