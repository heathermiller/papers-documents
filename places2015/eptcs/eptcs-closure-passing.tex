...
negatively affect their usage, maintenance, and optimization:
...
\item Typically, only high-level user-facing abstractions are statically
      ...
      \emph{maintenance} tasks, such as code refactorings, more difficult.
...
The following Section~\ref{sec:spores} provides some required background.
Sections~\ref{sec:model} (programming model) and~\ref{sec:impl} (implementation)
...
Spores~\cite{MillerHO14} are a closure-like abstraction and type system which aims to give users
...
problematic references. Moreover, importantly for object-oriented languages, it is no
...
\end{subfigure}%
\vspace{1mm}
\caption{The evaluation semantics of a spore is equivalent to that of a closure, obtained by simply leaving out the \texttt{spore} marker.}
...
systems such as Spark~\cite{Zaharia2012} in an effort to avoid problems with accidentally
captured references like \verb|this|. However, in systems like Spark, the
...
information of spores when they are composed.
...
support such patterns, our model includes a \emph{pumpTo} primitive for emitting
...
The \verb|apply| method takes a spore, a kind of closure (see Section~\ref{sec:spores} for an overview),
...
\item Once \verb|ref2|'s silo is materialized, its data is sent to the node executing the \verb|send|, completing \verb|fut|.
\end{enumerate}
\noindent
Note that since a \verb|send| operation sends the data of a silo across the network, it should only be invoked on silos containing small bits of data.
...
element type \verb|U| \newline (\verb|Traversable[U]|). Using \verb|pumpTo|, the elements (of type \verb|U|) of the
...
combined, yielding finally a \newline \verb|SiloRef[List[(K, (A, B))]]| as required.
...
experiments and explore the impact of implementation trade-offs. Furthermore, we plan
to exploit the simplicity of the programming model for a formal treatment of its
properties.
