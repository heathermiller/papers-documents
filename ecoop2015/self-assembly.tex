...
\titlerunning{Self-Assembly}  % abbreviated title (for running head)
...
\authorrunning{H. Miller, P. Haller, and B. C. D. S. Oliveira} % abbreviated author list (for running head)

\maketitle              % typeset the title of the contribution

\begin{abstract}
  In this paper we show a general technique, called \sselfassembly,
  for \emph{lightweight language extensions} (LLEs). LLEs allow
  ...
  serialization mechanism; or implement simple forms of custom pluggable type
  system extensions like an immutability checker. However unlike
...
\item\textbf{Performance}. Generic functions written by library authors or library
     users should have the same or better performance as language-integrated
     approaches with compiler/runtime support.
...
Since Scala/Pickling happens a majority at compile-time, serialization code is
fast and inlined, without requiring any boilerplate. Due to the fact that it
is completely based upon type classes, flexibility and extensibility come for
...
\begin{figure}[ht!]
  ...
  $\ba[t]{l@{\hspace{2mm}}ll}
T   & ::=     X~|~N                                 & \mbox{type} \\
% \\ & \\
N   & ::=     C[\seq{T}]                            & \mbox{non-variable type} \\
% \\ & \\
L   & ::=     mod^{*}~\texttt{class}~C[\seq{X<:N}](\seq{f:T})(\texttt{implicit}~\seq{g:S}) & \mbox{class definition}
\\
 & \gap ~\texttt{extends}~N(\seq{h})~\{~\seq{M}~\} & \\
% \\ & \\
M   & ::=     mod^{*}~\texttt{def}~m[\seq{X<:N}](\seq{x:T})(\texttt{implicit}~\seq{y:S}): T = e & \mbox{method definition} \\
% \\ & \\
mod & ::=   \texttt{sealed}~|~\texttt{final}~|~\texttt{implicit}  & \mbox{modifier} \\
% \\ & \\
e   & ::=   x~|~e.f~|~e.m[\seq{T}](\seq{e})~|~e.m[\seq{T}](\seq{e})(\seq{e'})~|~\texttt{implicitly}[T]  & \mbox{expression}
\\
& \gap ~~|~\texttt{new}~N(\seq{e})~|~\texttt{new}~N(\seq{e})(\seq{e'})~|~\texttt{new}~N(\seq{e})~\{~\seq{M}~\} &
\ea$
...
\begin{figure*}[ht!]
  \centering
\begin{mathpar}
\inferrule[\textsc{F-Top}]
{ \textit{fields}(\top)=\epsilon
}{}

\inferrule[\textsc{F-Class}]
{ \texttt{class}~C[\seq{X<:N}](\seq{f:T})(\texttt{implicit}~\seq{g:T})~\texttt{extends}~N(\seq{h})~\{\ldots\} \\
  \textit{fields}([\seq{T}/\seq{X}]N) = \seq{f':U}
}
{ \textit{fields}(C[\seq{T}]) = \seq{f':U},[\seq{T}/\seq{X}]~\seq{f:T}
}

\inferrule[\textsc{MT-Class}]
{ \texttt{class}~C[\seq{X<:N}](\seq{f:T})(\texttt{implicit}~\seq{g:T})~\texttt{extends}~N(\seq{h})~\{\seq{M}\} \\
  \texttt{def}~m[\seq{Y<:P}](\seq{x:U})(\texttt{implicit}~\seq{y:S}):U=e\in\seq{M}
}
{ \textit{mtype}(m,C[\seq{T}])=[\seq{T}/\seq{X}]([\seq{Y<:P}](\seq{U})(\seq{S})\Rightarrow U))
}

\inferrule[\textsc{MT-Super}]
{ \texttt{class}~C[\seq{X<:N}](\seq{f:T})(\texttt{implicit}~\seq{g:T})~\texttt{extends}~N(\seq{h})~\{\seq{M}\} \\
  m\notin\seq{M}
}
{ \textit{mtype}(m,C[\seq{T}]) = \textit{mtype}(m, [\seq{T}/\seq{X}]N)
}

\inferrule[\textsc{MB-Class}]
{ \texttt{class}~C[\seq{X<:N}](\seq{f:T})(\texttt{implicit}~\seq{g:T})~\texttt{extends}~N(\seq{h})~\{\seq{M}\} \\
  \texttt{def}~m[\seq{Y<:P}](\seq{x:U})(\texttt{implicit}~\seq{y:S}):U=e_o\in\seq{M}
}
{ \textit{mbody}(m[\seq{V}],C[\seq{T}])=(\seq{x})(\seq{y}).[\seq{T}/X][\seq{V}/Y]e_o
}

\inferrule[\textsc{MB-Super}]
{ \texttt{class}~C[\seq{X<:N}](\seq{f:T})(\texttt{implicit}~\seq{g:T})~\texttt{extends}~N(\seq{h})~\{\seq{M}\} \\
  m\notin\seq{M}
}
{ \textit{mbody}(m[\seq{V}],C[\seq{T}]) = \textit{mbody}(m[\seq{V}], [\seq{T}/\seq{X}]N)
}
\end{mathpar}
  % \vspace{-5mm}
  \caption{Field Lookup (F), Method Type Lookup (MT), and Method Body Lookup (MB)}
  \label{fig:lookup}
  % \vspace{-1mm}
\end{figure*}
...
\begin{figure*}[ht!]
  \centering
\begin{mathpar}
\inferrule[\textsc{S-Refl}]
{ \Delta \vdash T<:T
}{}

\inferrule[\textsc{S-Trans}]
{ \Delta \vdash S <: T \\
  \Delta \vdash T <: U
}{
  \Delta \vdash S <: U
...
\inferrule[\textsc{S-Var}]
{ \Delta \vdash X <: \Delta(X) \\
}{}

\inferrule[\textsc{S-Class}]
{ \texttt{class}~C[\seq{X <: N}](\seq{f:T})(\texttt{implicit}~\seq{g:T})~\texttt{extends}~N(\seq{h})~\{\ldots\}
}{\Delta \vdash C[\seq{T}] <: [\seq{T}/\seq{X}]N}

\inferrule[\textsc{WF-Top}]
{ \Delta \vdash \top~\mbox{ok}
}{}

\inferrule[\textsc{WF-Var}]
{ X \in \textit{dom}(\Delta)
}{ \Delta \vdash X~\mbox{ok}}

\inferrule[\textsc{WF-Class}]
{ \textit{mods}~\texttt{class}~C[\seq{X <: N}](\seq{f:T})(\texttt{implicit}~\seq{g:T})~\texttt{extends}~N(\seq{h})~\{\ldots\} \\
  \Delta \vdash \seq{T}~\mbox{ok} \\
  \Delta \vdash \seq{T} <: [\seq{T}/\seq{X}]\seq{N} \\
  \texttt{implicit} \notin \textit{mods} \\
  \{ \texttt{sealed}, \texttt{final} \} \nsubseteq \textit{mods}
}{ \Delta \vdash C[\seq{T}]~\mbox{ok}}

\end{mathpar}
  % \vspace{-5mm}
  \caption{Subtyping (S) and Well-formedness (WF)}
  \label{fig:subtyping-well-formedness}
  % \vspace{-1mm}
\end{figure*}
...
{ I ; \Delta ; \Gamma \vdash x : \Gamma(x)
}{}

\inferrule[\textsc{T-Field}]
{ I ; \Delta ; \Gamma \vdash e_0 : T_0 \quad fields(bound_{\Delta}(T_0)) = \seq{f : T}
}{
  I ; \Delta ; \Gamma \vdash e_0.f_i : T_i
}

\inferrule[\textsc{T-Invk}]
{ I ; \Delta ; \Gamma \vdash e_0 : T_0 \quad mtype(m, bound_{\Delta}(T_0)) = [\seq{Y <: P}](\seq{U})(\seq{S}) \Rightarrow U \\
  \Delta \vdash \seq{V}~ok \quad \Delta \vdash \seq{V} <: [\seq{V}/\seq{Y}]\seq{P} \quad I ; \Delta ; \Gamma \vdash \seq{e} : \seq{U'} \quad \Delta \vdash \seq{U'} <: [\seq{V}/\seq{Y}]\seq{U} \\
   \quad [\seq{V}/\seq{Y}]\seq{S} = \seq{S'} \quad \Gamma ; I \vdash \seq{p} : \seq{S'}
}{
  I ; \Delta ; \Gamma \vdash e_0.m [\seq{V}](\seq{e}) : [\seq{V}/\seq{Y}]U
}

\inferrule[\textsc{T-Invk-Ex}]
{ I ; \Delta ; \Gamma \vdash e_0 : T_0 \quad mtype(m, bound_{\Delta}(T_0)) = [\seq{Y <: P}](\seq{U})(\seq{S}) \Rightarrow U \\
  \Delta \vdash \seq{V}~ok \quad \Delta \vdash \seq{V} <: [\seq{V}/\seq{Y}]\seq{P} \quad I ; \Delta ; \Gamma \vdash \seq{e} : \seq{U'} \quad \Delta \vdash \seq{U'} <: [\seq{V}/\seq{Y}]\seq{U} \\
  I ; \Delta ; \Gamma \vdash \seq{e'} : \seq{S'} \quad \Delta \vdash \seq{S'} <: [\seq{V}/\seq{Y}]\seq{S}
}{
  I ; \Delta ; \Gamma \vdash e_0.m [\seq{V}](\seq{e})(\seq{e'}) : [\seq{V}/\seq{Y}]U
}

\inferrule[\textsc{T-New}]
{ \Delta \vdash C[\seq{V}]~ok \quad fields(C[\seq{V}]) = \seq{f : T} \\
  I ; \Delta ; \Gamma \vdash \seq{e} : \seq{S} \quad \Delta \vdash \seq{S} <: \seq{T} \\
  \texttt{class}~C[\seq{X <: N}](\ldots)(\texttt{implicit}~\seq{g : U})~\ldots \\
  [\seq{V}/\seq{X}]\seq{U} = \seq{U'} \quad \Gamma ; I \vdash \seq{p} : \seq{U'}
}{
  I ; \Delta ; \Gamma \vdash \texttt{new}~C[\seq{V}](\seq{e}) : C[\seq{V}]
}

\inferrule[\textsc{T-Method}]
{ \Delta = \seq{X <: N}, \seq{Y <: P} \quad \Delta \vdash \seq{T}, \seq{S}, T, P~ok \\
  \texttt{class}~C[\seq{X <: N}](\seq{f : U})(\texttt{implicit}~\seq{g : V})~\texttt{extends}~N(\seq{h}) \{ \ldots \} \\
  I = \seq{IVal(g, V)}, \seq{IVal(y, S)}, \textit{implicitDefs}(C[\seq{X <: N}]) \\
  I ; \Delta ; \seq{x : T}, \texttt{this} : C[\seq{X}] \vdash e_0 : S \quad \Delta \vdash S <: T
}{
  \texttt{def}~m[\seq{Y <: P}](\seq{x : T})(\texttt{implicit}~\seq{y : S}) : T = e_0~\text{OK}~\text{IN}~C[\seq{X <: N}]
}

\inferrule[\textsc{T-New-Anon}]
{ \texttt{class}~C[\seq{X <: N}](\seq{f : T})(\texttt{implicit}~\seq{g : S}) \ldots \{ \seq{M'}  \} \\
  I ; \Delta ; \Gamma \vdash \seq{e} : \seq{T'} \quad \Delta \vdash \seq{T'} <: \seq{T} \quad [\seq{V}/\seq{X}]\seq{S} = \seq{S'} \quad \Gamma ; I \vdash \seq{p} : \seq{S'} \\
  \forall~\texttt{def}~m[\seq{Y} <: \seq{P}](\seq{x: U})(\texttt{implicit}~\seq{y: U'}): R = e \in \seq{M} .\\~\texttt{override}(m, C[\seq{V}], [\seq{Y} <: \seq{P}](\seq{U})(\seq{U'}) \Rightarrow R)
}{
  I ; \Delta ; \Gamma \vdash \texttt{new} C[\seq{V}](\seq{e})\{ \seq{M}\} : C[\seq{V}]
}

\inferrule[\textsc{T-New-Ex}]
{ \Delta \vdash C[\seq{V}]~\mbox{ok} \quad \textit{fields}(C[\seq{V}]) = \seq{f: T} \\
  I; \Delta; \Gamma \vdash \seq{e} : \seq{S} \quad \Delta \vdash \seq{S} <: \seq{T} \\
  I; \Delta ; \Gamma \vdash \seq{e'} : \seq{S'} \quad \Delta \vdash \seq{S'} <: [\seq{V}/\seq{X}]\seq{U} \\
  \texttt{class}~C[\seq{X <: N}](\ldots)(\texttt{implicit}~\seq{g : U}) \ldots
}{
  I ; \Delta ; \Gamma \vdash \texttt{new}~C[\seq{V}](\seq{e})(\seq{e'}) : C[\seq{V}]
}
...
\begin{myverbbox}[\small]{\vappbody}
U.combine(
  U.combine(
    U.start(T),
    combinedFieldResults(T, x, U)
  )
)
\end{myverbbox}
...
\inferrule[\textsc{R-Gen}]
{ \texttt{generate}[TC[T]](U) \leadsto \texttt{new}~TC[T]~\{~\texttt{def}~apply(x: T): R = applyBody(T, x, U)~\}
}{}
...
\inferrule[\textsc{R-App}]
{ applyBody(T, x, U) \leadsto \vappbody
}{}


% \inference[\textsc{R-App}]{
%   \valph
% }{
%   test
% }
...
\end{mathpar}
  \caption{Rewriting rules}
  \label{fig:rewriting-rules}
\end{figure*}
