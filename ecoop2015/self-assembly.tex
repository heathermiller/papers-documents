...
% \\ & \\
L   & ::=     mod^{*}~\texttt{class}~C[\seq{X<:N}](\seq{f:T})(\texttt{implicit}~\seq{g:T}) & \mbox{class definition}
\\
 & \gap ~\texttt{extends}~N(\seq{h})~\{~\seq{M}~\} & \\
% \\ & \\
M   & ::=     mod^{*}~\texttt{def}~m[\seq{X<:N}](\seq{x:T})(\texttt{implicit}~\seq{y:T}) & \mbox{method definition} \\
% \\ & \\
mod & ::=   \texttt{sealed}~|~\texttt{final}~|~\texttt{implicit}  & \mbox{modifier} \\
% \\ & \\
e   & ::=   x~|~e.f~|~e.m[\seq{T}](\seq{e})(\seq{e'})  & \mbox{expression}
\\
& \gap ~~|~\texttt{new}~N(\seq{e})(\seq{e'})~|~\texttt{implicitly}[T] &
\ea$
