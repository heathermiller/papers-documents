...
% \\ & \\
e   & ::=   x~|~e.f~|~e.m[\seq{T}](\seq{e})~|~\texttt{implicitly}[T]  & \mbox{expression}
\\
& \gap ~~|~\texttt{new}~N(\seq{e})~|~\texttt{new}~N(\seq{e})~\{~\seq{M}~\} &
\ea$
...
{ I ; \Delta ; \Gamma \vdash e_0 : T_0 \quad mtype(m, bound_{\Delta}(T_0)) = [\seq{Y <: P}](\seq{U})(\seq{S}) \Rightarrow U \\
  \Delta \vdash \seq{V}~ok \quad \Delta \vdash \seq{V} <: [\seq{V}/\seq{Y}]\seq{P} \quad I ; \Delta ; \Gamma \vdash \seq{e} : \seq{U'} \\
  \Delta \vdash \seq{U'} <: [\seq{V}/\seq{Y}]\seq{U} \quad [\seq{V}/\seq{Y}]\seq{S} = \seq{S'} \quad I\langle\seq{S'}\rangle \vdash \seq{V}
}{
...
{ \Delta \vdash C[\seq{V}]~ok \quad fields(C[\seq{V}]) = \seq{f : T} \\
  ...
  [\seq{V}/\seq{X}]\seq{U} = \seq{U'} \quad I\langle\seq{U'}\rangle \vdash \seq{v}
}{
