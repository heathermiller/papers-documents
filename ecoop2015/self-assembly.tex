...
{ \texttt{class}~C[\seq{X <: N}](\seq{f : T})(\texttt{implicit}~\seq{g : S}) \ldots \{ \seq{M'}  \} \\
  I ; \Delta ; \Gamma \vdash \seq{e} : \seq{T'}\\
  \Delta \vdash \seq{T'} <: \seq{T} \\
  [\seq{v}/\seq{X}]\seq{S} = \seq{S'} \\
  \Gamma ; I \vdash \seq{p} : \seq{S'} \\
...
{ \Delta \vdash C[\seq{V}]~\mbox{ok} \\
  \textit{fields}(C[\seq{V}]) = \seq{f: T} \\
  I; \Delta; \Gamma \vdash \seq{e}: \seq{S} \\
  \Delta \vdash \seq{S} <: \seq{T} \\
  I; \Delta ; \Gamma \vdash \seq{e'} : \seq{S'} \\
  \Delta \vdash \seq{S'} <: [\seq{V}/\seq{X}]\seq{U} \\
  \texttt{class}~C[\seq{X <: N}](\ldots)(\texttt{implicit}~\seq{g : U}) \ldots
}{
