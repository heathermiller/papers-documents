...
\gap \texttt{def}~m(\seq{x: \sigma}): \phi = \texttt{rasync}~\{~mb~\}  & \mbox{async method} \\
...
\psi ::= \texttt{Task$[\sigma]$}                     & \mbox{taskable return type}    \\
\ea$
...
corresponding task object $\langle \texttt{Task}, state \mapsto
running(\bar{F}, \bar{D}) \rangle$. $\bar{F}$ is a set of asynchronous frames,
namely, all tasks that are currently suspended awaiting $o$ to publish a new
...
Note that the state of a task is $running(\bar{F}, \bar{D})$ as opposed to
