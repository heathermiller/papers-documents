...
\begin{itemize}
  \item  A design of a new programming model which integrates the Async model and the Reactive Extensions model in the context of the Scala Async project~\cite{ScalaAsync}~(which is proposed for adoption in mainline Scala~\cite{ScalaAsyncSIP});
...
The equivalent logic can be expressed using Scala Async as follows (this
example is adopted from the SIP proposal~\cite{ScalaAsyncSIP}):
