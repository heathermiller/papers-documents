...
\verb|awaitNext|, \verb|awaitDone|, and \verb|awaitFailure| to express waiting
for the events of an observable stream.
...
version of the \verb|async| construct of Section~\ref{sec:scala-async} (in
Scala implemented as a macro) which additionally supports methods to await
events of observable streams. The invocation of \verb|awaitNext| suspends the
\verb|rasync| block until the producer of \verb|stream| calls \verb|onNext| on
its observers. The argument of these \verb|onNext| calls (the next produced
event) is returned as a result from \verb|awaitNext|. The result of
\verb|rasync|, \verb|obs|, has type \verb|Observable[List[Int]]|. Once the
\verb|rasync| block has been evaluated to the end, the

[The result of rasync is an observable. Which events can be observed when?]

Sometimes it is not known statically how many events a stream might still
produce. One might want to collect all elements until the stream is done
(finished producing elements). In this case it is necessary to have a way to
wait for either of two events: the stream has a next element, or the stream is
done. This can be supported using a method \verb|awaitNextOrDone| which
returns an \verb|Option[T]| when applied to an \verb|Observable[T]|.
