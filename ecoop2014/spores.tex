...
\authorrunning{\hspace{-3cm} H. Miller, P. Haller, and M. Odersky} % abbreviated author list (for running head)
...
the popular ``big data'' framework Apache Spark.
Figure~\ref{fig:spark-apps-stats} shows a summary of the applications.\footnote{All
applications are hosted in public repositories on the social coding website GitHub.}
The applications comprise 66324 LOC (including comments and whitespace) of Scala source code.

\paragraph{\textbf{Methodology}} We evaluated the applications along the two
dimensions utility and practicality. In the first case we were interested how
often closures could benefit from the additional spore checking. Given that
all applications were using the Spark framework, we counted the number of
...
serializable to avoid runtime exceptions. (The \verb|RDD| type has many more
higher-order functions that require serializable closures such as
\verb|flatMap|; \verb|map| is by far the most common higher-order function,
though, and is thus in some sense representative of the use of closures in
Spark.) In the second case, we analyzed the percentage of spores that could be
converted automatically to spores assuming the Spark API would use spore types
instead of regular function types, thus not incurring any syntactic overhead.
In the case where such an automatic conversion would not be possible, we
analyzed the average number of captured variables, indicating the syntactic
overhead of using spores.

\paragraph{\textbf{Results}} Figure~\ref{fig:spark-apps-stats} summarizes our
results. As can be seen, the number of closures passed to the \verb|map|
method of the \verb|RDD| type varies between 5 (\verb|spark-jobserver|) and 81
(\verb|adam|). As mentioned before, this is only a fraction of the closures
that must be serializable, which is currently unchecked. Of all these
closures, about ??.?\% do not capture any variable; these closures could be
automatically converted to spores using the implicit macro presented in
earlier sections. The remaining ??.?\% of closures that do capture variables,
capture on average ??.? variables. Thus, converting such closures to spores
would not be an unreasonable burden, in particular using the lightweight
\verb|capture| syntax introduced earlier.
