...
\end{longtable}
\vspace{2mm}
\caption{Evaluating the impact and overhead of spores on real distributed applications. Each project listed is an active and noteworthy open-source project hosted on Github that is based on Apache Spark. \protect\stars~represents the number of ``stars'' (or interest) a repository has on Github, \protect\contribs~represents the number of open-source contributors to the project, and \protect\loc~represents the project's total lines of code.}
...
We have verified on an additional set of sources that excluding specific types
from closures is in fact important.
...
access the sender of a message, Akka's Actor trait provides a method
``sender'' that returns a reference to the actor that is the sender of the
...
sender, but a concurrent future-based computation invokes the ``sender''
...
method ``pipeTo'' to enable programming patterns using futures that avoid
capturing variables of type Actor in closures. However, the correct use of
``pipeTo'' is unchecked. This paper provides a new, but statically-checked
approach to address this problem by demanding closures passed to future
constructors to be spores with the constraint that type Actor is excluded.
