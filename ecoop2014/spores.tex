...
Each passing year, we iterate further and further from the single-machine
model of software development. The traditional view of a program running on a
single machine, interacting directly with the user is now obsolete. With the
growing trend towards cloud computing and mobile applications, distributed
programming has entered the mainstream.
...
asynchronous, reactive ways with other clients or services, even within the
same application.
...
traditionally imperative languages being extended with functional features,
such as Java~\cite{JavaLambdas}, C++~\cite{CplusplusLambas}, and \todo{add
more}, as well as the measurable popularity of functional programming
massively online open courses (MOOCs)~\cite{ICSEMOOCPaper} and perceived
importance in empirical studies on software
developers~\cite{PLAdoption}\todo{check Leo's paper, add others}.
