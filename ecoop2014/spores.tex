...
In Section~\ref{sec:sec:adv-usage-type-constraints} we informally describe the type
...
A small example is shown in Figure~\ref{fig:captured-ex}. (We omit the
\verb|Excluded| type member for simplicity; it is explained in detail
in Section~\ref{sec:excluded-types}.)
...
\begin{subfigure}{.5\textwidth}
  ...
  \end{lstlisting}
  \caption{A spore which captures a \texttt{String} and an \texttt{Int} in its spore header.}
  \label{fig:capture-spore}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \begin{lstlisting}
  ...
  \end{lstlisting}
  \caption{The corresponding spore type.}
  \label{fig:capture-type}
\end{subfigure}%
\vspace{1mm}
