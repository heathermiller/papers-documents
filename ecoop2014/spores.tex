...
\gap ~|~  x                                 & \mbox{variable}
\\
p ::=     (x: T) \Rightarrow t              & \mbox{abstraction}
\\
\gap ~|~  x~y                               & \mbox{application}
\\
\gap ~|~  \{ \seq{l = x} \}                 & \mbox{record construction}
\\
\gap ~|~  x.l                               & \mbox{selection}
\\
\gap ~|~  \texttt{spore}~\{~\seq{x : S = t}~; (x: T) \Rightarrow t~\}  & \mbox{spore}
\\
\gap ~|~  \texttt{import}~pn~\texttt{in}~t  & \mbox{import}
\\
\gap ~|~  t                                 & \mbox{term}
\\
 & \\
...
{ T_2 <: T_1 \andalso R_1 <: R_2 \\
  \seq{S'} \neq \texttt{Top} \andalso \seq{S} \subseteq \seq{S'} \\
  \seq{pn'} \subseteq \seq{pn}
}
...
\infrule[\textsc{S-SporeTop}]
{ T_2 <: T_1 \andalso R_1 <: R_2 \\
  \seq{pn'} \subseteq \seq{pn}
}
{ T_1 \Rightarrow R_1~\{~\texttt{type}~\mathcal{C} = \seq{S}~;~\seq{pn}~\} <: \\ T_2 \Rightarrow R_2~\{~\texttt{type}~\mathcal{C} = \texttt{Top}~;~\seq{pn'}~\}
}
...
{ T_1 \Rightarrow R_1~\{~\texttt{type}~\mathcal{C} = \seq{S}~;~\seq{pn}~\} <: T_2 \Rightarrow R_2
}
...
In the spore type we have a possibly-empty sequence of captured types. According to the subtyping rules, a spore with empty captured types is always a subtype as long as its closure types are compatible.
...
\infrule[\textsc{T-Let}]
{ \Gamma \vdash p : T_1 \quad \Gamma, x : T_1 \vdash t : T_2
}
{ \Gamma \vdash \texttt{let}~x = p~\texttt{in}~t : T_2
}
...
{ \Gamma \vdash \{ \seq{l = x} \} : \{ \seq{l : T} \}
}

\vspace{0.4cm}

\infrule[\textsc{T-Sel}]
{ \Gamma \vdash x : \{ \seq{l : T} \}
}
{ \Gamma \vdash x.l_i : T_i
}
...
{ \Gamma ; \Delta \vdash \texttt{spore}~\{~\seq{x : S = t}~; (x: T_1) \Rightarrow t~\} : \\
  \mathcal{S}~\{~\texttt{type}~\mathcal{C} = \seq{S}~;~\Delta~;~T_1 \Rightarrow T_2~\}
}

\vspace{0.4cm}

\infrule[\textsc{T-Comp}]
{ \Gamma \vdash t_1 : \mathcal{S}~\{~\texttt{type}~\mathcal{C} = \seq{S}~;~\Delta_1~;~T_1 \Rightarrow T_2~\} \\
  \Gamma \vdash t_2 : \mathcal{S}~\{~\texttt{type}~\mathcal{C} = \seq{R}~;~\Delta_2~;~U_1 \Rightarrow U_2~\} \\
  U_2 <: T_1 \\
  \Delta = \{ pn \in \Delta_1 \cup \Delta_2 ~|~ \seq{S} \subseteq P(pn) \land \seq{R} \subseteq P(pn) \}
}
{ \Gamma \vdash t_1~\texttt{compose}~t_2 : \mathcal{S}~\{~\texttt{type}~\mathcal{C} = \seq{S}, \seq{R}~;~\Delta~;~U_1 \Rightarrow T_2~\}
}
...
When type-checking we assume the existence of a global mapping from property names to type families, much like a global class table in a core language like FJ~\cite{FJ}.
