...
\title{Spores: Closures for Reliable Distributed and Concurrent Programming}
\titlerunning{Spores: Closures for Reliable Distributed and Concurrent Programming}  % abbreviated title (for running head)
...
C++11~\cite{CplusplusLambas}, and \todo{add more}, as well as the increase in
...
called {\em spores}, that can guarantee closures to be serializable, thread-safe, 
or even have general, custom user-defined properties. 
We implement our approach for Scala, evaluate its practicality through an 
small empirical study, and show the power of these guarantees through a 
case analysis of new distributed and concurrent frameworks that this safe 
foundation for migratable closures enables.
...
% called spores, that can guarantee closures to be serializable, thread-safe, 
% or even have custom user-defined properties. 
% We implement our approach for Scala, and show the 
% power of these guarantees through a case analysis of new distributed/concurrent 
% frameworks that this safe foundation for migratable closures enables.
...
\item We introduce a closure-like abstraction, called ``spores'', which avoids typical hazards when using closures in a concurrent or distributed setting through controlled variable capture and user-defined constraints for captured types.
...
%\item a description, general model, and type system of \textit{spores}, functions that are guaranteed to be 
%\item a general system for applying type-constraints to arbitrary properties beyond serializability or thread-safety.
%\item We demonstrate the practicality of our approach via a small empirical study, as well as a case analysis of new distributed and concurrent frameworks that this safe  foundation for migratable closures enables.
...
Scala's parallel collections~\cite{ProkopecBRO11} provide data-parallel operations for standard collection types like maps and sequences. These data-parallel operations typically take closures as arguments that are applied to all elements of the underlying collection in parallel. Unfortunately, it is easy to create race conditions by capturing mutable objects within such closures, and Scala's type checker does not provide any assistance for avoiding them.
