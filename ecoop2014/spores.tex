...
\\
\gap ~|~  \texttt{spore}^{\seq{pn}}~\{~\seq{x : T = v}~; (x: T) \Rightarrow t~\}  & \mbox{spore value}
\\
 & \\
...
{ \Gamma ; \Delta \vdash \texttt{spore}~\{~\seq{x : S = p}~; (x: T_1) \Rightarrow t~\} : \\
...
{ \Gamma \vdash x : T_1 \Rightarrow T_2~\{~\texttt{type}~\mathcal{C} = \seq{S}~;~\Delta_1~\} \\
  \Gamma \vdash y : U_1 \Rightarrow T_1~\{~\texttt{type}~\mathcal{C} = \seq{R}~;~\Delta_2~\} \\
  \Delta = \{ pn \in \Delta_1 \cup \Delta_2 ~|~ \seq{S} \subseteq P(pn) \land \seq{R} \subseteq P(pn) \}
}
{ \Gamma \vdash x~\texttt{compose}~y : U_1 \Rightarrow T_2~\{~\texttt{type}~\mathcal{C} = \seq{S}, \seq{R}~;~\Delta~\}
}
...
How do I distinguish the case where I capture one variable of type Any, and the case where I don't care about the captured variables?
...
{ \texttt{import}~pn~\texttt{in}~t \rightarrow attach(pn, t)
}
