...
[explain how type Capture[T] is used for type constraints]

For example, a spore from \verb|Int| to \verb|String| that requires all
captured types to be both thread-safe and serializable would have the
following type:
\begin{lstlisting}
    Spore[Int, String] {
      type Capture[T] = Safe[T] with Pickler[T]
    }
\end{lstlisting}
\noindent
The \verb|with| keyword is used to express the mix-in composition of multiple
traits.
...
\end{lstlisting}
\noindent
The above example defines a spore that requires an instance of type
\verb|Pickler[String]| for its captured type \verb|String|. The type of the
spore is thus:
