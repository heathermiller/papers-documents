...
def assert(x: Boolean, msg: String): Unit =
  macro assert_impl
def assert_impl(c: Context)(x: c.Expr[Boolean], msg: c.Expr[String]): c.Expr[Unit] = ...
\end{lstlisting}

\todo fix this lstlisting
...
a syntax tree of type \verb|Expr[Unit]|. It is \verb|assert_impl| which is
...
compiler. It is also important to note that implicit defs as described above
...
The transform macro implementation is passed the AST of the annotated class
