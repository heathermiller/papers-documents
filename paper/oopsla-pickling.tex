...
\footnotetext{TypeTags are part of the mainline Scala
...
\item to provide a survey of the properties of data types that are
  ...
  and
\item to evaluate the potential impact of our framework on code
...
The results are
shown in Figure~\ref{fig:results-vector} (a). As can be seen, Java is markedly slower than any other framework. This is likely due to the expensive runtime cost of the JVM's calculation of the runtime transitive closure of the objects to be serialied. For 1,000,000 elements, Java finishes in 491ms while Scala Pickling finishes in 86ms, or a factor 5.7 faster. As can be seen, the
...
lower than the need to support ad-hoc polymorphism (our framework
...
generics. Extensions that are orthogonal to the main notions of
picklers and pickler combinators include (a) support for retrofitting
existing types with pickling support, and (b) a composable model for
static pickler generation. Extension (a) can be achieved by a
type-class-like mechanism such as JavaGI~\cite{WehrT11}, C++
Concepts~\cite{ReisS06}, or, in our case, objects and implicits in
Scala~\cite{Oliveira2010}.

python pickle (good SO article). AliceML generic pickling and
minimization. rossberg's thesis.
...
concepts of object-oriented programming including ad-hoc polymorphism
