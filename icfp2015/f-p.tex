...
\item {\bf\em A distributed implementation of the programming model} in and for Scala.
...
\url{http://lampwww.epfl.ch/~hmiller/f-p/}\footnote{Review note: this site is publicly accessible and is linked to in talks and from other various pages. }
...
of Acute ML's ideas that extends to the OCaml bytecode compiler, and focuses
on type-safe serialization and providing globally meaningful type names. In
contrast, F-P merely a programming model, which does not require extensions to
the Scala compiler.
...
more of a middleware to facilitate the construction of such systems, and as a
result provides much finer-grained control over details such as fault handling
and network topology (\ie peer-to-peer vs master/worker).
...
building data-centric distributed systems. Thanks to a foundation consisting
of safe, serializable closures and type-safe serization,
\todo{evaluation point here}

In future work, we plan to better understand concerns of separate compilation
and apply our model to microservices.
