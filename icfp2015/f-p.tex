...
present a new {\em function-passing} (F-P) programming model designed to bring
a bit of order to this disarray. A goal of F-P is to provide a more principled
substrate on which to build data-centric distributed systems. A key approach
is to pass safe, well-typed functions to immutable distributed data, which in
many cases improves latency (keep data distributed) while making it easier by
design to recover from failures. We provide an operational semantics, an
implementation in and for Scala, and a small evaluation of the efficiency of
...
success of these frameworks.
% , from both the perspective of users and framework authors alike.
It's not hard to imagine why--a functional, declarative
interface to data distributed over tens to thousands of nodes is arguably most
natural to end-users and data scientists.
...
However, frameworks for distributed programming are built atop of tall stacks
of code which is typically imperative and untyped, losing most of the benefits
enjoyed by the users of their high-level APIs. This makes concerns central to
distributed systems such as concurrency and fault tolerance difficult to
achieve in practice.
...
\\
\gap ~|~  \texttt{map}(r, p)                & \mbox{map}
\\
\gap ~|~  \texttt{flatMap}(r, p)            & \mbox{flatMap}
\\
\gap ~|~  \texttt{send}(r)                  & \mbox{send}
\\
\gap ~|~  \texttt{await}(\iota)             & \mbox{await location}
\\
\gap ~|~  r                                 & \mbox{SiloRef}
\\
 & \\
...
\\
 & \\
r ::=     \text{Mat}(\iota) & \mbox{materialized SiloRef}
\\
\gap ~|~  \text{Mapped}(\iota, h, r, p)     & \mbox{mapped SiloRef}
\\
\gap ~|~  \text{FMapped}(\iota, h, r, p)    & \mbox{flatMapped SiloRef}
\\
 & \\
E      ::=  \epsilon & \mbox{message queue}
\\
\gap ~|~    \text{Res}(\iota, v) \texttt{::} E &
\\
\gap ~|~    \text{Req}(h, r, \iota) \texttt{::} E &
\\
  \ea$
...
\paragraph{Configurations.} The reduction rules of the deterministic layer define transitions of \emph{host configurations} $(t, E, S)^h$ of host $h$ where $t$ is a local redex term, $E$ is a message queue, and $S$ is a store. The reduction rules of the non-deterministic layer define transitions of sets $H$ of host configurations. The reduced host configurations are chosen non-deterministically.
...
{ (R[\texttt{await}(i_f)], E, S)^h \longrightarrow (R[\texttt{await}(i_f)], E', S')^h }
...
{ (R[\texttt{await}(P(i_f))], E, S)^h \rightarrow (R[\texttt{await}(P(i_f))], E'', S)^h
}
...
{ \{ (R[\texttt{send}(r)], E, S)^h, (t, E', S')^{h'} \} \cup H \rightarrow \{ (R[\texttt{await}(\iota)], E, S)^h, (t, E' \cdot m, S')^{h'} \} \cup H
}
...
{ E = \text{Req}(h'', r, \iota'') \texttt{::} E'' \quad r = \text{FMapped}(\iota, h, \text{Mat}(\iota_s), p) \quad S(\iota) = \text{None} \\
  S(\iota_s) = \text{Some}(v) \quad p(v) = r' \quad loc(r') = \iota' \quad S(\iota') = \text{None} \quad host(r') = h' \quad m = \text{Req}(h, r', \iota')
}
{ \{ (R[\texttt{await}(\iota_f)], E, S)^h, (t, E', S')^{h'} \} \cup H \rightarrow \{ (R[\texttt{await}(\iota_f)], E'', S)^h, (t, E' \cdot m, S')^{h'} \} \cup H
}
