...
like MapReduce are limited to.

To illustrate the decentralized nature of our model, consider the following
example: the local host would like to aggregate data as soon as it has been
materialized in two silos \verb|vehicles| and \verb|persons|, and write the
result of the aggregation to a file on a distributed file system:

\begin{lstlisting}
object Utils {
  def aggregate(vehicles: SiloRef[List[Vehicle]],
                persons: SiloRef[List[Person]]): SiloRef[String] =
    ...
...
val done = aggregate(vehicles, persons).map(spore {
  val localName = fileName
  (res: String) => Utils.write(res, localName)
})
...
This program does not tolerate faults of the local host: if it fails, the
result is never written to the file. Using fault handlers, though, it is
possible to introduce a backup host that takes over in case the local host
fails during the \verb|aggregate| step:
...
val hostb = Host(...) // backup host
val backup = SiloRef.fromFun(hostb)(spore {
  val captured = fileName
  () => captured
})

// dummy silo on local host for failure handling
val dummy = SiloRef.fromFun(localHost)(spore { () => true })

val checking = backup.flatMap(spore {
  val theVehicles = vehicles
  val thePersons  = persons
  val theDummy    = dummy
  (name: String) =>
    // when both silos are materialized...
    theVehicles.zip(thePersons).flatMap(spore {
      val localDummy = theDummy
      res =>
        // ... check if original host is still alive
        localDummy.map(identity)
    },
    spore { // fault handler
      res => backup.flatMap(todo)
    })
})

checking.cache() // kick off backup
todo(fileName).cache() // force computation
\end{lstlisting}
