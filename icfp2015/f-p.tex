...
\emph{lineage}~\cite{Lineage1,Linage2}. Essentially, the lineage of a data set
...
the data in the silo of the given SiloRef. Rather than immediately sending the
...
method called \verb|send| must be called explicitly to force the
materialization of the result silo. For example, given a silo with a list of
\verb|Person| records, the following application of \verb|map| defines a silo
containing only the records of adults:
...
initial data:\footnote{For clarity, only the method signatures are shown.}
