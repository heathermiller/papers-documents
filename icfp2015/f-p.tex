...
designed to bring a bit of order to this disarray. A goal of F-P is to provide
a more principled substrate on which to build data-centric distributed
systems. F-P seeks to
...
distributed systems such as concurrency and fault tolerance more difficult in
practice. Some of the several problems that negatively affect these systems'
usage, maintenance, and optimization include:

\begin{itemize}

\item Their APIs cannot statically prevent {\em common usage errors}. As a
result, users are often confronted with runtime errors that are hard to debug.
A common example is unsafe closure serialization~\cite{Spores}.
...
\item The absence of certain kinds of static type information precludes
systems-centric optimizations. Importantly, type-based static meta-programming
enables fast serialization~\cite{Pickling}, but this is only possible if also
lower layers (namely those dealing with object serialization) are statically
typed. Several studies~\cite{JavaSerialization, JavaRMI, MoreEfficientJavaRMI,
...
alternative serialization frameworks such as Protocol Buffers~\cite{Protobuf}, Apache
...
{ host(r) = h' \quad i~\text{fresh} \\
  r' = \text{Mapped}((h, i), h', r, p, p_f)
}
{ (R[\texttt{map}(r, p, p_f)], E, S)^h \longrightarrow (R[r'], E, S)^h }

\inferrule[\textsc{R-FMap}]
{ host(r) = h' \quad i~\text{fresh} \\
  r' = \text{FMapped}((h, i), h', r, p, p_f)
}
{ (R[\texttt{flatMap}(r, p, p_f)], E, S)^h \longrightarrow (R[r'], E, S)^h }
...
{ E = \text{Req}(h', r, \iota'') \texttt{::} E' \quad r = \text{Mapped}(\iota, h, r', p, p_f) \quad r' \neq \text{Mat}(\iota_s) \quad S(\iota) = \text{None} \\
...
{ E = \text{Req}(h', r, \iota') \texttt{::} E' \quad r = \text{Mapped}(\iota, h, r', p, p_f) \quad r' = \text{Mat}(\iota_s) \quad S(\iota) = \text{None} \\
...
{ E = \text{Req}(h'', r, \iota'') \texttt{::} E'' \quad r = \text{FMapped}(\iota, h, \text{Mat}(\iota_s), p, p_f) \quad S(\iota) = \text{None} \\
...
{ E = \text{Req}(h'', r, \iota'') \texttt{::} E'' \quad r = \text{FMapped}(\iota, h, \text{Mat}(\iota_s), p, p_f) \quad S(\iota) = \text{None} \\
  S(\iota_s) = \text{Some}(v) \quad p(v) = r' \quad host(r') = h' \quad \text{failed}(h') \quad p_f(v) = r_f \quad host(r_f) = h_f \quad \not \text{failed}(h_f) \\
  loc(r_f) = \iota_f \quad S(\iota_f) = \text{None} \quad m = \text{Req}(h, r_f, \iota_f)
}
