...
distributed systems (how to build these higher-order operations is described
in Section~\ref{sec:higher-order-operations}). These primitives include:
...
semantics. The main principle is to specify fault handlers for \emph{subgraphs
of computation DAGs}.

The main principle is to allow code interacting with a SiloRef to
respond to a failure of the corresponding silo. We say that a silo has failed
if its host node has become unreachable or unresponsive. In our model,
failures are detected \emph{lazily} whenever an operation involving a failed
silo is attempted to be completed. For example, calling \verb|send| on a
SiloRef pointing to a failed silo would lead to the detection of a failure.

The definition of fault handling
