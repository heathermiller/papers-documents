...
distributed programming.\todo{include description of our implementation of babyspark!}
...
\paragraph{map} The \verb|map| method takes a spore that is to be applied to
the data in the silo associated with the given SiloRef. Rather than
immediately sending the spore across the network, and waiting for the
operation to finish, the \verb|map| method is \emph{lazy}. Without involving
any network communication, it immediately returns a SiloRef referring to a
new, lazily-created silo. This new SiloRef only contains lineage information,
namely, a reference to the original SiloRef, a reference to the argument
spore, and the information that it is the result of a \verb|map| invocation.
As we explain below, another method, \verb|send| or \verb|cache|, must be
called explicitly to force the materialization of the result silo.

To better understand how DAGs are created and how remote silos are
materialized, we will follow a running example. Given a silo containing a list
of \verb|Person| records, the following application of \verb|map| defines a
silo containing only the records of adults:
...
to the data in the silo of the given SiloRef. However, the crucial
...
magnitude greater than accessing a word in main memory, say, we felt an
explicit send operation a judicious choice.

To enable materialization of remote silos to proceed concurrently, the
\verb|send| operation immediately returns a future~\cite{Future}. This future
...
The performance of typical data analytics jobs can be increased dramatically
by caching large data sets in memory~\cite{Spark}. For this, the silo
containing the data set needs to be materialized. So far, the only way to
...
requesting node--too much if a large remote data set should merely be cached
in memory. Therefore, an additional \verb|primitive| called ``cache'' is
provided, which forces the materialization of the given SiloRef, returning
a trivial \verb|Future[Boolean]| that is completed as soon as the
corresponding silo has been materialized in memory. Example:
...
with the trivial value of the \verb|DoneSiloRef| singleton object (e.g., a
Boolean constant). Essentially, invoking \verb|send| on this trivial SiloRef
causes the resulting future to be completed as soon as \verb|this| SiloRef has
been materialized in memory.
