...
2. Implicit class, or implicit conversion, kicks in which provides a dummy `pickle` method which takes an implicit `Pickler` as a parameter.
...
contains the logic for actually pickling the object `x` (the constructor
parameter of `PickleOps`). The `Pickler[T]` can be generated at compile
...
First, we need to have the following defs in scope:
    ...
    def genPicklerImpl[...](c: Context): c.Expr[Pickler[T]]
...
not be enough to guarantee the proper type upon unpickling:
    ...
    // this would generate a Pickler[Person]
    def send(p: Person): Unit = sendOverWire(p.pickle)
...
First, the object to be pickled must somehow be viewable as
...
**welcome, as this is a rather complicated solution.**
