...
This paper presents a new programming model\footnote{We consider a programming 
model as an abstraction of the underlying computer system that allows for the 
expression of both algorithms and data structures.} called the {\em function 
passing} model which has been designed to be a more principled substrate (or 
middleware) upon which to build data-centric distributed systems. It can be 
viewed as a generalization of the MapReduce/Spark programming model--though it 
is not limited to the MapReduce/Spark programming model alone as we will later 
show.
...
closures over the network. \todo[inline]{"The key idea behind the function 
passing model...". Sounds like it's our idea to keep data
stationary and instead send functionality over the wire. But reading "White -
2012 - Hadoop The Definitive Guide" and "Zaharia - 2015 - Learning Spark", both
resources state at the very beginning --- resumed in my own words: Whereas
traditional parallelism brings the data to the compute, [Hadoop MapReduce |
Apache Spark] does the opposite--it brings the compute to the data.\\[2ex]

We adopt bringing compute to data but elevate communication robustness and 
performance via our unique combination of --- already exisiting artifacts --- 
pickles and spores with a novel concept of monadic silos.}. This enables two 
important benefits for distributed system builders; (a) since all computations 
are functional transformations on immutable data, fault-tolerance is made simple 
by design, and (b) communication is made well-typed by design, a common pain 
point for builders of distributed systems. Said another way, the function 
passing model attempts to more naturally model the paradigm of data-centric 
programming by extending monadic programming to the network. On this note, one 
might observe that the function passing model can actually be interpreted as 
somewhat of a dual to the actor model;\footnote{There are many variations and 
interpretations of the actor model; in saying our model is somewhat of a dual, 
we simply mean to highlight that programmers need not focus on programming with 
typically stationary message handlers. Instead, our model focuses on a monadic 
interface for programming with data (and sending functions instead).} rather 
than keeping functionality stationary and sending data, in our model, we keep 
data stationary and send functionality to the data.\todo[inline]{Here, once 
again, I --- as a "normal" reader --- get the impression that the authors are 
presenting a new model that brings compute to data, but this is already known to 
me by Hadoop and Spark.}
...
semantics of functional fault recovery. The fault-recovery mechanisms
of widespread systems such as Apache Spark, MapReduce~\cite{MapReduce} and
...
recovery in a {\emph{purely functional}} setting--to our knowledge a
novelty in the PL literature.
...
\item {\bf\em A formalization of our programming model} based on a small-step
  ...
  fault recovery mechanisms. The presented semantics is clearly stratified
  into a deterministic layer and a concurrent/distributed layer. Importantly,
...
\item {\bf\em A distributed implementation of the programming model} in and for
  Scala as a middleware. In addition, we present prototype versions we have
  built of popular frameworks like Apache Spark and MBrace \todo{Reference?} 
  using the function passing model, and end-user applications we have built
  using each of these prototype frameworks.
...
\todo[inline]{That's why I want to clarify programming model. It's not only 
about data structures but also about the algorithms utilizing those data 
structures. Maybe "In the broadest sense, the function passing model can be 
understood as a persistent data structure with monadic operations."}. However, 
rather than containing pure data, instead the data structure represents a 
directed acyclic graph of functional transformations on distributed data. The 
root node contains immutable data read from stable storage (\eg Amazon S3); 
edges represent functional transformations on immutable data represented as 
nodes of the DAG. \todo[inline]{Maybe to clarify matters, after defining various 
transformations starting from the root note, those child nodes are placeholders 
for the data resulting from the to be executed transformations.}
...
distributed systems~\cite{FlumeJava}.
...
\verb|map| method makes use of deferred evaluation; it {\emph{eventually}} 
applies a user-defined function to data pointed to by the SiloRef, creating in a 
new silo containing the result of this application. That is, this computation is 
only kicked off when the \verb|send| method is invoked. This makes it possible 
to queue up transformations in order to optimize network communication. Like 
\verb|map|, the application of \verb|flatMap| is deferred. \verb|flatMap| also 
applies a user-defined function to data pointed to by the SiloRef; unlike 
\verb|map|, however, the user-defined function passed to \verb|flatMap| returns 
a SiloRef whose contents is transferred to the new silo returned by 
...
\todo[inline]{In case of we are talking about the concept, I'd call it 
"reference to silo" or "silo reference", and in case of we are talking about the 
representation within our realization I'd call it SiloRef and always put in 
{\textbackslash}verb. Will not markup further usage of "SiloRef" till we've 
aggreed on.}
...
\todo[inline]{Peter J. Landin defined the term closure in 1964 as having an 
environment part and a control part. Joel Moses credits Landin with introducing 
the term closure to refer to a lambda expression whose open bindings (free 
variables) have been closed by (or bound in) the lexical environment, resulting 
in a closed expression, or closure. Cf. https://goo.gl/xe9ohw\\[2ex] Therefore 
--- as to me --- "Spores" are the {\em{substantiation of closures}}, rather than 
closure-like.}
...
avoids accidentally capturing problematic references \todo[inline]{Maybe we 
could add "e.g. SparkContext"}. Moreover, importantly for object-oriented 
...
internal framework class). \todo[inline]{Would that be on the signature of `map` 
or rather in the respective spores header? Maybe a short code snippet could 
clarify matters?}

For a deeper understanding of spores, see
% either Appendix~\ref{appendix:spores} which covers the semantics of spores or
the corresponding publication~\cite{Spores}. \todo[inline]{I'd remove this 
citation, as being redundant to the one at the beginning of this paragraph}
...
Figure~\ref{fig:basic-diagram} \todo[color=gray]{BTW, which tool did you use to 
  create those diagrams?}(time flows vertically from top to bottom).
...
the primitive operations available on SiloRefs and their semantics.
...
to build higher-order operations common to popular data-centric distributed 
systems (how to build some of these higher-order operations is described in 
Section~\ref{sec:higher-order-operations}). These primitives include:
...
to-be-created silo. This new SiloRef only contains lineage information, namely, 
a reference to the original SiloRef, a reference to the argument spore, and the 
information that it is the result of a \verb|map| invocation. As we explain 
below, another method, \verb|send| or \verb|cache|, must be called explicitly to 
force the materialization of the result silo.

To better understand how DAGs are created and how remote silos are
materialized, we will develop a running example \todo[inline]{Do we have a Gist 
for that? Or at least have the implemenation in the samples directory of our f-p
repository?} throughout this section. Given
...
%\inline{As this is an illustrative example, I'd fully annotate the types, i.e., 
%adults: SiloRef[List[Person]]`}
...
\verb|vehicles| SiloRef in its spore header. The spore header spans all variable 
definitions between the spore marker and the parameter list of the spore's 
closure. The spore header defines the variables that the spore's closure is 
allowed to access. Essentially, spores limit the free variables of their 
closure's body to the closure's parameters and the variables declared in the 
spore's header. \todo[inline]{Spores have been already explained} Within the 
spore's closure, it is necessary to read the data of the silo referenced by 
\verb|vehicles| in addition to the \verb|ps| list of \verb|Person| records. This 
requires calling \verb|map| on \verb|localVehicles|. However, \verb|map| returns 
a SiloRef; thus, invoking \verb|map| on \verb|adults| instead of \verb|flatMap| 
would be impossible, since there would be no way to get the data out of the silo 
returned by \verb|localVehicles.map(..)|. With the use of \verb|flatMap|, 
however, the call to \verb|localVehicles.map(..)| creates the final result silo, 
whose data is then also contained in the silo returned by \verb|flatMap|.
...
a {\emph{silo reference proxies}}, delegating to the actual data:
...
Figure~\ref{fig:bigger-dag}. \todo[inline]{Isn't in the figure the silo of 
`owners` missing?} Note that \verb|vehicles| is not a regular parent in the 
lineage of \verb|owners|; it is an indirect input used to compute \verb|owners| 
by virtue of being {\em captured} by the spore used to compute \verb|owners|. 
\todo[inline]{What is a "regular parent"?}
...
\verb|fromFun| method takes a spore as an argument to make sure it can be 
serialized and sent to \verb|Host|. In each case, the returned SiloRef contains 
its \verb|Host| as well as a host-unique identifier. \todo[inline]{Why do we 
mention theses constituents of a SiloRef?} The \verb|fromLineage| method is 
particularly interesting as it creates a copy of a previously existing silo 
based on the lineage of a SiloRef \verb|s|. Note that only the SiloRef is 
necessary for this operation to successfully complete; the silo originally 
hosting \verb|s| might already have failed.
...
\todo[inline]{Here we leave the reader alone; at least a short description re 
operation fusion applied should be added}
...
\verb|Unit|). \todo[inline]{The `send` forces flatMap's spore to be send to the 
silo referened by `this`. Then `res => SiloRef[Unit]()` is applied to the data 
in the silo creating a new silo with value `()` in it. How does this enforce 
that the silo referenced by `this` is now in memory at the respective host?} 
Essentially, invoking \verb|send| on this trivial SiloRef causes the resulting 
future to be completed as soon as \verb|this| SiloRef has been materialized in 
memory.
...
The introduced primitives enable expressing surprisingly intricate \todo{Why 
"intricate"?} computational patterns.
...
\todo[inline]{What is the introduction of this section telling the author?}
...
required, e.g. \verb|combined.map(spore { ts => (merge compose sort) ts })|.
...
\end{itemize}
\todo[inline]{Here we should add a footnote with a link to the actual 
implementation}
...
So far, our examples have focused on master-worker topologies underlying
...
Here, client \#1 would like to perform some sort of computation based on a 
collection of \verb|Vehicle| records. Another client, client \#2, would also 
like to access this dataset\todo{data set or dataset?}. To do so, one must 
simply once again invoke \verb|fromTextFile| on the same host, 
\verb|Host("lmpsrv1.scala-lang.org",9999)| to obtain a SiloRef that points to a 
corresponding silo that is already or soon to be materialized. From here, client 
\#2 is able to build an entirely different DAG of computations, for instance in 
this example, filtering the original \verb|vehicles| dataset to obtain only 
vehicles manufactured since 2013.
...
calculus with records. Figure~\ref{fig:syntax} shows the abstract syntax of our 
core language. Terms are standard except for the \texttt{spore}, \texttt{map}, 
\texttt{flatMap}, \texttt{send}, and \texttt{await} terms. A \texttt{spore} term 
creates a new spore. It contains a list of variable definitions (the spore 
header) and the spore's closure. A term $\texttt{await}(\iota)$ blocks execution 
until the future $\iota$ has been completed asynchronously. The \texttt{map}, 
