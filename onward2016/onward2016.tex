...
\fixme{However, these frameworks are built atop of tall stacks of typically imperative
...
We present a new programming model we call {\em function passing} designed to
overcome \fixme{most of these issues by providing a more principled substrate}
on which to build data-centric distributed systems. The function passing model
...
data-centric programming by extending monadic programming to the network. It can
be thought of as somewhat of a dual to the actor model;\footnote{There are many
...
model, we keep data stationary and send functionality to the data. This results
in well-typed communication by design, a common pain point for builders of
distributed systems in Scala.
...
systems. \fixme{Interestingly, we found that deferred evaluation was an enabler
in our model, without complicating the ability to reason about programs. Without
optimizations based on deferred evaluation, we found this model would be
impractically inefficient in memory and time.}
...
\item {\bf\em A new data-centric programming model for functional processing of
  ...
  design. \fixme{The main principle is immutable data stored in silos, persistent
  data structure that evolves} The main computational principle is based on the
  ...
  computations, supporting decentralized distributed computations. Deferred
  evaluation enables important optimizations while keeping programs simple to
  reason about.
...
\item {\bf\em A distributed implementation of the programming model} in and for
  Scala. \fixme{We present experiments that show some of the benefits of the proposed
...
\paragraph{Essence} In the broadest sense, the function passing model can be
thought of in terms of a persistent functional data structure with structural
sharing. However, rather than containing pure data, instead the data structure
represents a directed acyclic graph (DAG) of functional transformations on
distributed data. The root node .; the nodes in the DAG are merely which are
able to , and edges transformations.
...
Central to our model is the careful use of deferred evaluation. Computations on
...
\paragraph{} The function passing model consists of three main components:
\begin{itemize}[noitemsep]
...
function passing includes overloaded variants of the primitives discussed so far which
...
function passing is compiled and run using Scala 2.11.5, and considers only the JVM backend
...
The design leading up to function passing has incorporated many similar ideas, such as
...
as a remote invocation (pickling arguments and result appropriately). In function passing,
...
inspired by Erlang. In contrast to Termite, function passing is statically typed, enabling
...
critical for the flatMap combinator in function passing. In contrast to parallel closures,
...
and rebinding, and versioning. function passing, in contrast, is based on spores, which ship
...
names. In contrast, function passing merely a programming model, which does not require
...
Dryad~\cite{Dryad} are just that--distributed systems. function passing is meant to act as
...
agent's state. function passing, in contrast, proposes that data in stationary containers be
...
shared memory scenario, whereas function passing is designed with remote references for a
...
We have presented function passing, a new programming model and principled substrate for
...
We have also shown that function passing is able to express patterns of computation richer
...
build different sorts of distributed frameworks and applications atop function passing in an
