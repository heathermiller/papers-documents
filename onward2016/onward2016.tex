...
grow in importance, what can we as language designers \todo[inline]{Sounds like
we propose improvements to those languages; F-P is more an interlayer between
such a language and the frameworks to enable better framework design} do to make
...
middleware\todo{Yes!}) upon which to build data-centric distributed systems. It
...
closures over the network. \todo[inline]{Sounds like it's our idea to keep data
...
Apache Spark] does the opposite--it brings the compute to the data. So, here, we
should emphasize the term "closure" to hint the reader to our actual key idea.}
...
Scala \todo{Just "in Scala"?}. Said another way, the function passing model
...
and send functionality to the data.\todo[inline]{Here, once again, I --- as a
"normal" reader --- get the impression that the authors are presenting a new
model that brings compute to data, but this is already known to me by Hadoop and
Spark.}
...
The function passing model brings together immutable, persistent 
\todo{"immutable" and "persistent", aren't those terms equivalent? 
https://goo.gl/eXXA5K} data structures, monadic higher-order functions, strong 
static typing, and deferred evaluation--pillars of functional programming--to 
provide a more type-safe, and easy to reason about foundation upon which to 
build data-centric distributed systems. Interestingly, we found that deferred 
evaluation was an enabler in our model, without complicating the ability to 
reason about programs as we will soon show. Without optimizations based on 
deferred evaluation, we found this model would be impractically inefficient in 
memory and time.\todo[inline]{The emphasis on "lazyness" confuses me a bit. 
Isn't lazyness inherent to FP rsp. a consequence of the property of referential 
transparency? Said in other words, don't we get that for free, alone due to the 
fact that be base our model on the FP paradigm?} 

One important contribution of our model is a precise specification of the
semantics of functional fault recovery. The \todo[inline]{Here I would
substitute "The" with "This" or emphasize the term "functional". Otherwise it
sounds to me that we are specifying a "functional fault recovery" oposed to the
fault recovery mechanism of those widespread system.} fault-recovery mechanisms
...
recovery in a purely functional \todo[inline]{Highlighting "purely functiona"
would make out point clear re my previous comment.} setting--to our knowledge a
...
\item {\bf\em A new data-centric programming model for functional processing of
  ...
  graphs \todo[inline]{First usage of "directed acyclic graphs", so we could
  already here introduce the abbreviation "DAG".} of computations, supporting
  decentralized distributed computations. Deferred evaluation enables
  important optimizations while keeping programs simple to reason
  about.\todo[inline]{Is the term "programming model" well-understood? Maybe a
  footnote "We consider a programming model as an abstraction of the
  underlying computer system that allows for the expression of both algorithms
  and data structures". In case of the former we utlize monadic operations and
  in case of the latter we introcude Silos}
...
\item {\bf\em A distributed implementation of the programming model} in and for
  ...
  built of popular frameworks like Spark \todo[inline]{We always called it
  "Apache Spark" so far} and MBrace \todo{Reference?} using the function
  passing model, and end-user applications we have built using each of these
...
structures.}. However, rather than containing pure data, instead the data
structure represents a directed acyclic graph (DAG) of functional
transformations on distributed data. The root node of is \todo{s/of
is/contains/} immutable data read from stable storage (\eg Amazon S3); edges
represent functional transformations on immutable data represented as nodes of
the DAG. \todo[inline]{Maybe to clarify matters, after defining various
transformations starting from the root note, those child nodes are "empty"/
placeholders/ ... till forcing execution.}
...
DAG.\todo[inline]{If I mentally substitute "lineage" by "subgraph" --- as just 
introduced --- this sentence is weird. Therefore I'd leave out "represented by 
...
applying a function to distributed data just creates an immutable \todo{local?} 
lineage. To make a network \todo{remote?} call and thus obtain the result of a 
computation, it is necessary to first ``kick off'' computation, or to force 
\todo{the traversal of?} its lineage. Within our programming model, this force 
...
Similar to a proxy object, a SiloRef \todo{put in "{\textbackslash}verb"?} 
represents, and allows interacting with, both local and remote silos. SiloRefs 
\todo{put in "{\textbackslash}verb"?}
...
"reference to silo", and in case of we are talking about the representation 
within our realization I'd call it SiloRef and put in {\textbackslash}verb. Will 
not markup further usage of "SiloRef" till we've aggreed on this suggestion.} 
...
evaluation; it \todo{eventually? and leave out ", though ... deferred} applies a 
...
\todo{I'd would leave out "or stage"; just take one "queue up" or "stage"} 
transformations in order to optimize network communication. Like \verb|map|, the 
application of \verb|flatMap| is deferred. \verb|flatMap| \todo{also?} applies a 
...
whose contents are \todo{content is?} transferred to the new silo returned by 
...
\todo[inline,color=red]{To clarify matters: Let's assume we have a silo at a 
remote machine `m` referenced by a silo ref: `r1[int] --> s1[int]@m`. Now we 
have a function `f :: int -> SiloRef[str]` which based on integers creates a new 
silo at a remote machine `n`: `r2[str] --> s2[str]@n`. Flatmapping `f` on 
`r1[int]` results in `r3[str] --> s3[str]@m`. First `f` creates a silo on `n` 
and then `flatMap` transfers `s2[str]@n` to `m` yielding `s3[str]@m` with 
`s2[str] == s3[str]`. For sure, `m` and `n` might be equal. All in all `flatMap`
is for \emph{sequencing} operations operating within the respective 
...
Peter J. Landin defined the term closure in 1964 as having an environment part 
and a control part.

Joel Moses credits Landin with introducing the term closure to refer to a lambda 
expression whose open bindings (free variables) have been closed by (or bound 
in) the lexical environment, resulting in a closed expression, or closure. 

Cf. https://goo.gl/xe9ohw

Therefore --- as to me --- "Spores" are the substantiation of closures, rather 
than closure-like.
}
...
functions (as spores) to SiloRefs, which \todo[inline]{This "which" could be 
understood to refer to "SiloRefs". So, I'd rephrase to: Users interact with this 
distributed data by applying functions (as spores) to SiloRefs. Those functions
are transmitted over the wire and later ... } are transmitted over the wire and 
...
data via a SiloRef, a \todo{new?} SiloRef representing a new silo containing the 
...
function is asynchronously sent over the wire and is \todo{then?} applied to 
...
these in the following section. \todo[inline]{If we run low on space, this last 
sentence could be dismissed.}
...
to build the \todo{I'd leave off the "the"} higher-order operations common to 
...
these primitives in the context of a running example \todo[inline]{I'd leave off 
this sentence as it is redundant to the paragraph subsequent to `map`}. These 
primitives include:
...
\todo[inline]{To save space, I'd inline this itemization, like (0) map, (1) 
flatMap, ...}
...
soon-to-be-created \todo{Nice, but I'd leave off "soon"} silo. This new SiloRef 
...
\end{lstlisting}
\todo[inline]{Code comments should be incorporated into text if aggreed; maybe 
not here but within the paragraph of `send`.}
\noindent
Note that the spore passed to \verb|flatMap| declares the capturing of the 
\verb|vehicles| SiloRef in its so-called ``spore header.'' \todo{Why 
"so-called"?} The spore header spans all variable definitions between the spore 
...
the variables declared in the spore's header. \todo[inline]{Spores have been 
already explained} Within the spore's closure, it is necessary to read the data 
...
provided, which forces the \todo{remote} materialization of the given SiloRef, 
...
\todo[inline]{Isn't in the figure the silo of `owners` missing?}
...
\verb|fromFun| method takes a spore closure\todo{remove "closure"} as an 
...
identifier. \todo[inline]{Why do we here mention theses constituents of a 
SiloRef?} The \verb|fromLineage| method is particularly interesting as it 
...
Don't understand this caching.} Essentially, invoking \verb|send| on this 
...
required. \todo{One additional map?}
...
\todo{Here we should add a footnote with a link to the actual implementation}
