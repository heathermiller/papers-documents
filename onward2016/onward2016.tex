...
Every lineage originates from a materialized silo, represented as a value
$Mat(\omega)$ in our formal model (see Figure~\ref{fig:syntax}). Here, $\omega$
is the silo's identifier such that $\omega = (h, i)$ where $h$ is a host
identifier and $i$ is an integer that is unique on host $h$. Assume that a
materialized silo $Mat(\omega)$ is bound to a variable $x$; then the following
expression creates an extended lineage based on the \verb|map| combinator:

\begin{lstlisting}
val z = map(x, (y: T) => ...)
\end{lstlisting}
\noindent
In the formalization $z$ would then be bound to the lineage $Mapped(\omega',
Mat(\omega), (y: T) => ...)$ where $\omega' = (h, i')$, i.e., the resulting silo
is materialized on the same host $h$ as silo $\omega$.

Since new lineage identifiers can be created locally, creating lineages does not
require network communication. Therefore, the reduction rules for creating
lineages are integrated with regular, local reduction.
...
Rules \textsc{R-Map}, \textsc{R-FMap}, and \textsc{R-Persist} describe the
creation of lineages. Rules \textsc{R-Map} and \textsc{R-FMap} create lineages
using the constructors {\em Mapped} and {\em FMapped}, respectively. The new
lineage has a fresh identifier $(h, i)$ which uniquely identifies the
corresponding (logical) silo. In each case, the spore value $p$ is stored in the
new lineage; this enables a materialization of the silo identified by $(h, i)$
using parent lineage $r$ and spore $p$. Rule \textsc{R-Persist} creates a
lineage using the {\em Persist} constructor. {\em Persist} contains a function
($\cdot \cup \cdot$) enabling host $h$ to persist silo $r$. (Our full
formalization~\cite{FPTechReport} also includes a corresponding rule for
unpersisting a silo.)

As we can see, the above rules do not execute their corresponding higher-order
functions; they merely build lineages. A lineage contains everything necessary
to materialize the contents of its corresponding silo. Moreover, lineages are
serializable: as a result, they can be used to initiate the materialization of a
silo located on a remote node.

The immutability of silos and lineages enables equational reasoning about
programs, a cornerstone of functional programming. Lineages are also persistent
datastructures, where multiple lineages are able to share common ancestors,
thereby avoiding duplicate materializations. Taken together, we believe that
lineages in general, and the function passing model in particular are a natural
fit for functional programming, and lend themselves for embedding in a
functional programming language (see Section~\ref{sec:implementation}).
