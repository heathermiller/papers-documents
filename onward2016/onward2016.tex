...
Figure~\ref{fig:seq-reduction} shows select rules for sequential
...
The annotation with host $h$ is used for creating {\em identifiers}
$\omega = (h, i)$ for lineages/silo references.
...
enabling host $h$ to persist ($\cdot \cup \cdot$) silo $r$,
respectively. (Our full formalization~\cite{FPTechReport} also
includes a corresponding rule for unpersisting a silo.)
