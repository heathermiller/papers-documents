...
would be impractically inefficient in memory and time.\todo[inline]{The emphasis
on "lazyness" confuses me a bit. Isn't lazyness inherent to FP rsp. a
consequence of the property of referential transparency? Said in other words,
don't we get that for free, alone due to the fact that be base our model on the
FP paradigm?} 
...
\item {\bf\em A new data-centric programming model for functional processing of
  distributed data} which makes important concerns like fault tolerance simpler
    ...
    graphs of computations, supporting decentralized distributed computations.
    Deferred evaluation enables important optimizations while keeping programs
    simple to reason about.\todo[inline]{Is the term "programming model"
    well-understood? Maybe a footnote "We consider a programming model as an
    abstraction of the underlying computer system that allows for the expression
    of both algorithms and data structures". In case of the former we utlize
    monadic operations and in case of the latter we introcude Silos}
