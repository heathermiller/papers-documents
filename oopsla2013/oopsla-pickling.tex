...
F ::= & [] ~|~ F[x \mapsto r]    & \mbox{environment}        \\
H ::= & [] ~|~ H[r \mapsto o]    & \mbox{heap}               \\
r \in & RefLocs                  & \mbox{reference location} \\
o ::= & (T, F)                   & \mbox{object}
\ea$
\caption{Environments, heaps, and objects.}
...
introduce picklers as first-class objects, and provide their contracts that
valid implementations must guarantee. Subsequently, we
...
semantics for a core object-oriented language. Expressions of the core
language are standard except for the addition of two special \texttt{pickle}
and \texttt{unpickle} expressions. Importantly, our goal is not a full
formalization of a core language; instead, we aim to provide a more precise
definition of object-oriented picklers. Thus, we omit reduction rules for
standard expression forms, and we show only reduction rules for the
\texttt{pickle} and \texttt{unpickle} expressions.

The operational semantics uses the definitions shown in
Figure~\ref{fig:heaps-objects}. A configuration is composed of a heap $H$ and an
expression $e$. The dynamic representation of an object is a tuple $(T, F)$
containing a type tag $T$ for its dynamic type and an environment $F$ for the
values of its fields. A heap $H$ maps reference locations (abstract heap addresses)
to objects.

\begin{defn}(First-Class Picklers)

A {\em pickler} for some type $T$ is a tuple $((p, u), m, T)$ where $T$ is a
type tag, $m$ is the pickler's kind, and $(p, u)$ is a pair of two functions.
A pickler's kind can be either $s$ (static) or $d$ (dynamic).
