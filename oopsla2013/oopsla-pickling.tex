...
framework for generating them at compile-time. While specifically
...
In this section we introduce picklers as first-class objects, and, using
examples, motivate the contracts that valid implementations must guarantee.
Subsequently, we provide a formal model of object-oriented picklers based on
an operational semantics. Finally, we demonstrate that the introduced picklers
enable modular, object-oriented pickler combinators, \ie methods for composing
more complex picklers from simpler primitive picklers.
...
\begin{figure*}
  ...
  \begin{subfigure}{0.3\textwidth}
    ...
    \infrule[\textsc{Pickle-S}]
    { \rho = ((p, u), s, T) \\
      o = (T, F)            \\
      r_p \notin H \quad H' = H[r_p \mapsto p(o)]
    }
    { \reduction H {\texttt{pickle}(\rho, o)} {H'} {r_p} }
  ...
  \begin{subfigure}{0.3\textwidth}
    ...
    \infrule[\textsc{Unpickle}]
    { \rho = ((p, u), m, T) \\
      o = (P, F)            \\
      r \notin H \quad H' = H[r \mapsto u(o)]
    }
    { \reduction H {\texttt{unpickle}(\rho, o)} {H'} r }

  \end{subfigure}
  \caption{Reduction rules for \texttt{pickle} and \texttt{unpickle} expressions.}
  \label{fig:opsem-rules}
\end{figure*}


To define picklers formally we use an approach based on a standard operational
...
meth ::=  \texttt{def}~m(\seq{x: C}): D = t      & \mbox{method}              \\
t    ::=  \texttt{let}~x = p~\texttt{in}~t       & \mbox{let binding}         \\
\gap ~|~ x.l := y                                & \mbox{assignment}          \\
\gap ~|~ x                                       & \mbox{variable}            \\
p    ::=  \texttt{new}~C(\seq{x})                & \mbox{instance creation}   \\
\gap ~|~ x.f                                     & \mbox{selection}           \\
\gap ~|~ x.m(\seq{y})                            & \mbox{invocation}          \\
\gap ~|~ \texttt{pickle}(x, y)                   & \mbox{pickle}              \\
\gap ~|~ \texttt{unpickle}(x, y)                 & \mbox{unpickle}            \\
\gap ~|~ t                                       & \mbox{term}                \\
\ea$
...
sequence of class definitions followed by a (main) term. Without loss of
generality, we use a form where all intermediate terms are named (A-normal
form~\cite{Flanagan93}). Terms $t$ are standard except for the addition of two
special \texttt{pickle} and \texttt{unpickle} term forms. The language does
not support arbitrary mutable variables ({\em cf.}~\cite{TAPL}, Chapter~13);
instead, only fields of objects can be (re-)assigned.
...
\label{fig:heaps-objects}
\end{figure}

The operational semantics uses the definitions shown in
Figure~\ref{fig:heaps-objects}. An environment $F$ maps field names to reference
locations. A heap
$H$ maps reference locations to values. In our core language, values can be
either objects or picklers. For reasons discussed below, a pickler is not a
regular object. The dynamic representation of an object is a pair $(C, F)$
containing a class type tag $C$ for its dynamic type and an environment $F$
for its fields. In contrast, a pickler is a tuple as defined in
Definition~\ref{def:pickler} below.
...
(u \circ p)(o) = o' \Rightarrow o \equiv o'
\end{equation}

\end{defn}

As defined above, picklers are first-class, since they are values just like
objects. However, while picklers are regular objects in our practical
implementation, picklers are different from objects in the present formal
model. The reason is that a pickler has to contain a type tag indicating the
types of objects that it can pickle (this is apparent in the rules of the
operational semantics below); however, the alternative of adding parameterized
types (as in, \eg, FGJ~\cite{Igarashi2001}) is beyond the scope of the paper.
...
Two picklers are structurally-equal, written $\rho \equiv \rho'$ {\em iff}

$\rho = ((p, u), m, C) \land \rho' = ((p', u'), m', C') \land m = m' \land C = C'$
